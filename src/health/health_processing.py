...
from src.health.apple_processing import full_apple_pipeline
from src.nutrilio.nutrilio_processing import full_nutrilio_pipeline
from src.location.location_processing import full_location_pipeline
from src.screentime.offscreen_processing import full_offscreen_pipeline
...
def load_apple_minute_data():
    ...
    """
    apple_path = 'files/processed_files/apple/apple_processed.csv'
...
def load_nutrilio_mental_health():
    ...
    """
    nutrilio_path = 'files/processed_files/nutrilio/nutrilio_processed.csv'

    if not os.path.exists(nutrilio_path):
        print(f"❌ Nutrilio file not found: {nutrilio_path}")
        return None

    print(f"🥗 Loading Nutrilio mental health data...")
    df = pd.read_csv(nutrilio_path, sep='|', encoding='utf-8')

    # Filter only daily summary rows
    df_daily = df[df['daily_summary'] == 'yes'].copy()
...
def load_screentime_data():
    ...
    """
    screen_path = 'files/processed_files/screentime/offscreen_processed.csv'
...
def create_health_files():
    ...
    try:
        ...
        health_dir = 'files/processed_files/health'
...
def full_health_pipeline(auto_full=False, auto_process_only=False):
    ...
    if success:
        ...
        record_successful_run('health_coordination', 'coordination')
    else:
