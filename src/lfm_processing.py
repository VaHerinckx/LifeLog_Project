...
def compute_completion(df):
    ...
    for i in range(1, len(df)):
        ...
        if time_diff >= timedelta(minutes=duration):
            df.at[i, 'skip_next_track'] = time_diff.total_seconds() / 60
...
def artist_info(token, artist_names, artist_df):
    ...
    for artist_name in artist_names:
        if artist_name in dict_artists.keys():
            pass
        else:
            count += 1
...
def track_info(token, song_keys, track_df):
    ...
    for song_key in song_keys:
        if song_key in dict_tracks.keys():
            pass
        else:
...
def process_lfm_export():
    ...
    unique_artists = list(df.artist_name.unique())
    ...
    df['new_recurring_artist_yn'] = df.groupby('artist_name').cumcount() == 11
    df['new_track_yn'] = df.groupby('track_name').cumcount() == 0
    df['new_recurring_track_yn'] = df.groupby('track_name').cumcount() == 6
