...
def update_file(file_name):
    """Function to update a single file in Google Drive with better error handling"""
    try:
...
def update_drive(file_names):
    """Function to update multiple files in Google Drive with better error handling"""
    try:
        drive = get_authenticated_drive()
        fileList = drive.ListFile({'q': f"'{folder_id}' in parents and trashed=false"}).GetList()
        ...
        for file_name in file_names:
            try:
                ...
                if not file_found:
                    failed_updates.append(f"{file_name} (not found)")

            except Exception as e:
                failed_updates.append(f"{file_name} ({str(e)})")
