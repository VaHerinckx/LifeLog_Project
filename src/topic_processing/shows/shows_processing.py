...
def create_shows_topic_file():
    ...
    Returns:
        bool: True if successful, False otherwise
    """
    print("⚙️  Creating Shows topic files...")
    ...
    try:
        ...
        if not os.path.exists(source_file):
            ...
            print("   Run the Trakt source pipeline first.")
            return False
        ...
        print(f"❌ Error creating Shows topic files: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def upload_shows_results():
    ...
    """
    print("☁️  Uploading Shows results to Google Drive...")
    ...
    if success:
        print("✅ Shows results uploaded successfully!")
    else:
...
def full_shows_pipeline(auto_full=False, auto_process_only=False, skip_source=False):
    """
    ...
    print("\n" + "="*60)
    if success:
        print("✅ Shows topic pipeline completed successfully!")
        # Record successful run
        record_successful_run('topic_shows', 'active')
    else:
        print("❌ Shows topic pipeline failed")
    print("="*60)
...
if __name__ == "__main__":
    ...
    print("📺 Shows Topic Processing Tool")
    print("This tool coordinates Shows data sources and generates website files.")
