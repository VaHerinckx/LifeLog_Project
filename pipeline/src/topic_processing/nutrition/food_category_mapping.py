...
def load_food_mapping():
    ...
    try:
        if os.path.exists(FOOD_MAPPING_FILE):
            ...
            log.warning(f"Warning: Food mapping file not found: {FOOD_MAPPING_FILE}")
            log.info(f"Create this file with format: food_item|food_category")

        return FOOD_MAPPING

    except Exception as e:
        log.error(f"Error loading food mapping: {e}")
        return {}
...
def load_drink_mapping():
    ...
    try:
        if os.path.exists(DRINK_MAPPING_FILE):
            ...
            log.warning(f"Warning: Drink mapping file not found: {DRINK_MAPPING_FILE}")
            log.info(f"Create this file with format: drink_item|drink_category")

        return DRINK_MAPPING

    except Exception as e:
        log.error(f"Error loading drink mapping: {e}")
        return {}
...
def analyze_unmapped_items(df, top_n=50):
    ...
    # Analyze FOOD items
    log.info("\n" + "="*60)
    log.progress("️ Food Category Analysis")
    log.info("="*60)
    ...
    if 'food' in df.columns:
        ...
        log.success(f"Loaded {len(FOOD_MAPPING)} mappings | {len(unique_categories)} categories | {coverage_pct:.1f}% coverage")
        log.info(f"Mapped: {mapped_items:,} items | Unmapped: {unmapped_items:,} items")
    ...
    if 'drink' in df.columns:
        ...
        log.success(f"Loaded {len(DRINK_MAPPING)} mappings | {len(unique_categories)} categories | {coverage_pct:.1f}% coverage")
        log.info(f"Mapped: {mapped_items:,} items | Unmapped: {unmapped_items:,} items")
        log.progress(f"\n Current Categories: {', '.join(unique_categories)}")
        ...
        if unmapped_items > 0:
            ...
            # Print LLM prompt
            log.info("\n" + "━"*60)
            log.info("LLM PROMPT FOR DRINK CATEGORIZATION")
            log.info("━"*60)
            log.info("\nMap these drink items to ONE of these categories:")
            log.info(f"{', '.join(DRINK_CATEGORIES)}")
        ...
        log.warning("No 'drink'column found in DataFrame")
...
if __name__ == '__main__':
    ...
    try:
        ...
        log.success(f"Food: {stats['food_mappings']} mappings  {len(stats['food_categories'])} categories")
        log.info(f"Categories: {', '.join(stats['food_categories'])}")

        log.success(f"\n Drinks: {stats['drink_mappings']} mappings  {len(stats['drink_categories'])} categories")
        log.info(f"Categories: {', '.join(stats['drink_categories'])}")
        ...
        if os.path.exists(processed_file):
            ...
            log.info(" Run the Nutrition topic coordinator pipeline first to generate data")
            log.info("\n To generate data, run:")
            log.info(" python src/topic_processing/nutrition/nutrition_processing.py")
