    ...
    if os.path.exists(GEOCODING_CACHE_FILE):
        try:
            with open(GEOCODING_CACHE_FILE, 'r', encoding='utf-8') as f:
                cache = json.load(f)
            log.progress(f"Loaded geocoding cache with {len(cache)} locations")
            return cache
        except Exception as e:
            log.warning(f"Error loading geocoding cache: {e}")
            return {}
    return {}
...
def save_geocoding_cache(cache: Dict[str, Dict[str, str]]) -> None:
    ...
    try:
        ...
        log.success(f"Saved geocoding cache with {len(cache)} locations")
    except Exception as e:
        log.warning(f"Error saving geocoding cache: {e}")
...
def reverse_geocode_coordinates(lat: float, lon: float) -> Dict[str, str]:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            log.warning(f"Geocoding failed for {lat}, {lon}: HTTP {response.status_code}")
            return {
                'city': 'Unknown City',
        ...
        log.warning(f"Error geocoding {lat}, {lon}: {e}")
        return {
            'city': 'Unknown City',
            'country': 'Unknown Country',
            'place_name': None,
...
def parse_coordinates(geo_string: str) -> Tuple[float, float]:
    ...
    try:
        ...
        log.warning(f"Error parsing coordinates from '{geo_string}': {e}")
        return 0.0, 0.0
...
def extract_timezone_from_timestamp(timestamp_str: str) -> str:
    ...
    try:
        ...
        log.warning(f"Error extracting timezone from '{timestamp_str}': {e}")
        return "UTC+00:00"
...
def geocode_location(city: str, country: str) -> Tuple[str, str]:
    ...
    try:
        ...
        log.progress(f"Geocoding {city}, {country}...")
        ...
        if response.status_code == 200:
            ...
            if data and len(data) > 0:
                ...
                log.success(f"Found coordinates: {coordinates}")
                ...
                log.error(f"No results found for {city}, {country}")
                return "0.0,0.0", "UTC"
        else:
            log.error(f"Geocoding failed: HTTP {response.status_code}")
            return "0.0,0.0", "UTC"

    except Exception as e:
        log.error(f"Error geocoding {city}, {country}: {e}")
        return "0.0,0.0", "UTC"
...
def get_timezone_from_coordinates(lat: float, lon: float, city: str, country: str) -> str:
    ...
    """
    log.progress(f"Getting timezone for coordinates {lat}, {lon}...")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if timezone_id != 'UTC':
                log.success(f"Found timezone via geographic rules: {timezone_id}")
                return timezone_id

    except Exception as e:
        log.info(f"WorldTimeAPI failed: {e}")

    # Method 2: Try GeoNames with shorter timeout and better error handling
    try:
        log.progress("Trying GeoNames API...")
        ...
        if response.status_code == 200:
            ...
            if timezone_id and timezone_id != 'unknown':
                log.success(f"Found timezone via GeoNames: {timezone_id}")
                return timezone_id

    except Exception as e:
        log.info(f"GeoNames failed: {e}")

    # Method 3: Geographic rules based on coordinates (most reliable)
    log.progress("Using geographic coordinate rules...")
    timezone_id = get_timezone_by_geographic_rules(lat, lon, country)
    if timezone_id != 'UTC':
        log.success(f"Found timezone via coordinate rules: {timezone_id}")
        return timezone_id

    # Method 4: Enhanced country fallback
    log.progress("Using enhanced country fallback...")
...
def get_timezone_fallback(country: str, city: str) -> str:
    ...
    log.info(f"Using fallback timezone: {timezone_id}")
    return timezone_id
...
def calculate_timezone_offset(timezone_id: str, date: datetime) -> str:
    ...
    try:
        ...
        log.warning(f"Error calculating timezone offset for {timezone_id} on {date}: {e}")
        return "UTC+00:00"
