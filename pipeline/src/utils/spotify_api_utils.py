...
def get_artist_info(token, artist_names, artists_work_file):
    ...
    for artist_name in artist_names:
        ...
        if str(artist_name).lower() in dict_artists.keys():
            ...
            count += 1
            log.progress(f"New artist {count}: {artist_name}")
            dict_info_artists = {}
            ...
            try:
                ...
                if response.status_code == 429:
                    ...
                    log.warning(f"Rate limited by Spotify. Waiting {retry_after} seconds...")
                ...
                if not response.text or response.text.strip() == '':
                    log.warning(f"Empty response for artist: {artist_name}, skipping")
                ...
                if not response.text or response.text.strip() == '':
                    log.warning(f"Empty response for artist details: {artist_name}, skipping")
                    dict_info_artists['followers'] = "No API result"
                    dict_artists[str(artist_name).lower()] = dict_info_artists
                    continue
                ...
                log.error(f"JSON parsing error for artist '{artist_name}'")
                log.debug(f"Error: {e}")
                log.debug(f"Response status: {response.status_code}")
                log.debug(f"Response headers: {dict(response.headers)}")
                log.debug(f"Response body (first 500 chars): {response.text[:500]}")
                dict_info_artists['followers'] = "API Error - JSON Parse Failed"
                dict_artists[str(artist_name).lower()] = dict_info_artists
                continue

            except requests.exceptions.RequestException as e:
                log.error(f"Network error for artist '{artist_name}': {e}")
                dict_info_artists['followers'] = "Network Error"
                dict_artists[str(artist_name).lower()] = dict_info_artists
                continue

            except Exception as e:
                log.error(f"Unexpected error for artist '{artist_name}': {e}")
            ...
            if count % 50 == 0:
                df_artist = pd.DataFrame.from_dict(dict_artists, orient='index').reset_index().rename(columns={'index':'artist_name'})
                df_artist.to_csv(artists_work_file, sep='|', index=False)
    log.milestone(f"{count} new artist(s) added to artist dictionary")
    df_artist = pd.DataFrame.from_dict(dict_artists, orient='index').reset_index().rename(columns={'index':'artist_name'})
...
def get_track_info(token, song_keys, tracks_work_file):
    ...
    for song_key in song_keys:
        ...
        if (str(song_key).lower() in dict_tracks.keys()) | (song_key == ''):
            ...
            count += 1
            log.progress(f"Info gathered for {count}/{count_API_requests} new songs")
            dict_info_tracks = {}
            ...
            try:
                ...
                # Validate response before parsing JSON
                if not response.text or response.text.strip() == '':
                    log.warning(f"Empty response for track: {song_key}, skipping")
                ...
                # Validate second API call response
                if not response.text or response.text.strip() == '':
                    log.warning(f"Empty response for track details: {song_key}, skipping")
                    dict_info_tracks['track_duration'] = "No API result"
                    dict_tracks[str(song_key).lower()] = dict_info_tracks
                    continue
                ...
                if response.status_code == 200 and response.text and response.text.strip() != '':
                    try:
                        ...
                        log.info(f"Could not parse audio features for {song_key}")

            except requests.exceptions.JSONDecodeError as e:
                log.error(f"JSON parsing error for track '{song_key}'")
                log.debug(f"Error: {e}")
                log.debug(f"Response status: {response.status_code}")
                ...
                log.error(f"Network error for track '{song_key}': {e}")
                dict_info_tracks['track_duration'] = "Network Error"
                dict_tracks[str(song_key).lower()] = dict_info_tracks
                continue

            except Exception as e:
                log.error(f"Unexpected error for track '{song_key}': {e}")
    ...
    log.milestone(f"{count} new track(s) added to track dictionary")
    df_track = pd.DataFrame.from_dict(dict_tracks, orient='index').reset_index().rename(columns={'index':'song_key'})
