...
def time_difference_correction(df: pd.DataFrame, timestamp_column: str, source_timezone: str = 'GMT', debug: bool = False) -> pd.DataFrame:
    ...
    if debug:
        ...
        log.debug(f"Timestamp column: {timestamp_column}")
        log.debug(f"Source timezone: {source_timezone}")
        log.debug(f"Available columns: {list(df.columns)}")

        if timestamp_column in df.columns:
            sample_values = df[timestamp_column].head()
            log.debug(f"Sample timestamp values: {sample_values.tolist()}")
            log.debug(f"Timestamp column dtype: {df[timestamp_column].dtype}")
            log.debug(f"Has any null values: {df[timestamp_column].isnull().any()}")
        else:
            log.debug(f"TIMESTAMP COLUMN '{timestamp_column}' NOT FOUND!")
    ...
    if df.empty:
        log.info("Empty DataFrame provided, returning as-is")
        return df.copy()
    ...
    def parse_timezone_offset(tz_str: str) -> int:
        ...
        log.warning(f"Could not parse timezone: {tz_str}, assuming UTC")
        return 0
    ...
    def smart_timestamp_conversion(series: pd.Series) -> pd.Series:
        ...
        if len(sample_values) == 0:
            log.info("No valid timestamp values found")
        ...
        if pd.api.types.is_datetime64_any_dtype(series):
            log.debug("Timestamps are already datetime objects")
    ...
    log.progress(f"Starting timezone correction for {len(df)} records...")
    log.progress(f"Source timezone: {source_timezone}")
    log.debug(f"Timestamp column: {timestamp_column}")

    # Validate source timezone
    try:
        source_offset_minutes = validate_and_parse_timezone(source_timezone)
        log.success(f"Source timezone validated: {source_offset_minutes/60:+.1f} hours from UTC")
    ...
    try:
        ...
        log.success(f"Loaded location data: {len(location_df)} records")

        # DEBUG: Check location data immediately after loading (only if debug enabled)
        if debug:
            log.debug("RAW LOCATION DATA:")
            log.debug(f"Location columns: {list(location_df.columns)}")
            if 'timestamp' in location_df.columns:
                sample_loc_timestamps = location_df['timestamp'].head()
                log.debug(f"Raw location timestamps: {sample_loc_timestamps.tolist()}")
                log.debug(f"Raw location timestamp dtype: {location_df['timestamp'].dtype}")

        has_location_data = True

    except FileNotFoundError:
        log.warning(f"Location file not found: {location_file_path}")
        log.info("Proceeding with simple timezone conversion (no location-based correction)")
    ...
    if has_location_data:
        ...
        if missing_columns:
            log.warning(f"Location data missing required columns: {missing_columns}")
            log.info("Proceeding with simple timezone conversion")
            has_location_data = False
    ...
    if has_location_data:
        ...
        if debug:
            log.debug("PARSING LOCATION TIMESTAMPS:")
            log.debug(f"Before parsing: {location_df['timestamp'].dtype}")
            log.debug(f"Sample raw values: {location_df['timestamp'].head().tolist()}")
            log.debug(f"Type of first value: {type(location_df['timestamp'].iloc[0])}")
        ...
        try:
            ...
            if debug:
                log.debug(f"After initial parsing: {location_df['timestamp_parsed'].dtype}")
                log.debug(f"Sample parsed values: {location_df['timestamp_parsed'].head().tolist()}")
                if hasattr(location_df['timestamp_parsed'].dtype, 'tz'):
                    log.debug(f"Timezone info: {location_df['timestamp_parsed'].dtype.tz}")
            ...
            if hasattr(location_df['timestamp_parsed'].dtype, 'tz') and location_df['timestamp_parsed'].dtype.tz is not None:
                ...
                log.debug("Location timestamps have timezone info, converting to UTC...")
                ...
                if '+' in first_ts or first_ts.count('-') > 2:  # Has timezone offset
                    log.debug("Parsing timestamps with timezone information...")
                    ...
                    if hasattr(location_df['timestamp_parsed'].dtype, 'tz') and location_df['timestamp_parsed'].dtype.tz is not None:
                        ...
                        log.debug("Timezone in string not detected by pandas, converting manually")
                        location_df['timestamp_parsed'] = pd.to_datetime(location_df['timestamp'], errors='coerce', utc=True).dt.tz_localize(None)
                else:
                    log.debug("Location timestamps are already timezone-naive (UTC assumed)")

            if debug:
                log.debug(f"Final location timestamp dtype: {location_df['timestamp_parsed'].dtype}")
                log.debug(f"Final sample (should be UTC): {location_df['timestamp_parsed'].head().tolist()}")

        except Exception as e:
            log.error(f"Error parsing location timestamps: {e}")
            log.info("Falling back to simple timezone conversion")
            import traceback
            traceback.print_exc()
            has_location_data = False
        ...
        if has_location_data:
            ...
            try:
                ...
                if hasattr(location_df['timestamp_parsed'].dtype, 'tz') and location_df['timestamp_parsed'].dtype.tz is not None:
                    log.debug("Converting timezone-aware timestamps to naive...")
                    ...
                    log.debug("Ensuring all timestamps are timezone-naive...")
                ...
                log.debug(f"Error ensuring timezone-naive: {e}")
            ...
            log.progress(f"Location data range: {location_df['timestamp_parsed'].min()} to {location_df['timestamp_parsed'].max()}")

            # DEBUG: Show sample of location data
            if debug:
                log.debug("Sample location data:")
                log.debug(f"{location_df[['timestamp', 'timestamp_parsed', 'timezone', 'local_offset_minutes']].head()}")
                log.debug(f"Location timestamp dtype: {location_df['timestamp_parsed'].dtype}")
    ...
    if debug:
        log.debug("SMART CONVERSION INPUT:")
        log.debug(f"Input dtype: {result_df[timestamp_column].dtype}")
        log.debug(f"Input sample: {result_df[timestamp_column].head().tolist()}")

    result_df['temp_timestamp_parsed'] = smart_timestamp_conversion(result_df[timestamp_column])

    if debug:
        log.debug("SMART CONVERSION OUTPUT:")
        log.debug(f"Output dtype: {result_df['temp_timestamp_parsed'].dtype}")
        log.debug(f"Output sample: {result_df['temp_timestamp_parsed'].head().tolist()}")
        if hasattr(result_df['temp_timestamp_parsed'].dtype, 'tz'):
            log.debug(f"Output timezone: {result_df['temp_timestamp_parsed'].dtype.tz}")

        # DEBUG: Show sample of input timestamp data
        log.debug("Sample input timestamp data:")
        log.debug(f"Original values: {result_df[timestamp_column].head().tolist()}")
        log.debug(f"Original dtype: {result_df[timestamp_column].dtype}")
        log.debug(f"Parsed values: {result_df['temp_timestamp_parsed'].head().tolist()}")
        log.debug(f"Parsed dtype: {result_df['temp_timestamp_parsed'].dtype}")
    ...
    if debug:
        ...
        log.debug("Sample UTC conversion:")
        log.debug(f"UTC values: {result_df['temp_utc_timestamp'].head().tolist()}")
        log.debug(f"UTC dtype: {result_df['temp_utc_timestamp'].dtype}")
        if hasattr(result_df['temp_utc_timestamp'].dtype, 'tz'):
            log.debug(f"UTC timezone: {result_df['temp_utc_timestamp'].dtype.tz}")
    ...
    if not has_location_data:
        ...
        log.info("Using simple timezone conversion (no location-based adjustments)")
        ...
        log.success(f"Successfully converted {len(result_df)} timestamps")
        return result_df

    # Location-based timezone correction (only when location data is available)
    log.progress("Applying location-based timezone corrections...")

    # CRITICAL FIX: Ensure both DataFrames have timezone-naive timestamps before any operations
    if debug:
        log.debug("FINAL TIMEZONE CHECK BEFORE COMPARISON:")
        log.debug(f"Input UTC dtype: {result_df['temp_utc_timestamp'].dtype}")
        log.debug(f"Location parsed dtype: {location_df['timestamp_parsed'].dtype}")
    ...
    if hasattr(result_df['temp_utc_timestamp'].dtype, 'tz') and result_df['temp_utc_timestamp'].dtype.tz is not None:
        if debug:
            log.debug("Removing timezone from input UTC timestamps...")
        result_df['temp_utc_timestamp'] = result_df['temp_utc_timestamp'].dt.tz_localize(None)
        if debug:
            log.debug(f"Input UTC after fix: {result_df['temp_utc_timestamp'].dtype}")

    if hasattr(location_df['timestamp_parsed'].dtype, 'tz') and location_df['timestamp_parsed'].dtype.tz is not None:
        if debug:
            log.debug("Removing timezone from location timestamps...")
        location_df['timestamp_parsed'] = location_df['timestamp_parsed'].dt.tz_localize(None)
        if debug:
            log.debug(f"Location after fix: {location_df['timestamp_parsed'].dtype}")

    # Show sample values for final comparison
    if debug:
        log.debug(f"Sample input UTC: {result_df['temp_utc_timestamp'].head().tolist()}")
        log.debug(f"Sample location: {location_df['timestamp_parsed'].head().tolist()}")

    # OPTIMIZED APPROACH: Use merge_asof instead of apply for massive performance boost
    # This is O(n log n) instead of O(n*m)
    log.progress("Using optimized merge for timezone matching...")
    ...
    if merged['local_offset_minutes'].isna().any():
        ...
        log.info(f"{merged['local_offset_minutes'].isna().sum()} timestamps before location data, using offset {first_offset/60:+.1f}h")
    ...
    log.success(f"Successfully corrected {total_corrections} timestamps")

    if total_corrections > 0:
        log.debug("Sample corrections:")
        sample_size = min(5, len(df))

        for i in range(sample_size):
            original = df.iloc[i][timestamp_column]
            corrected = result_df.iloc[i][timestamp_column]
            log.debug(f"{original}  {corrected}")

    log.success("Timezone correction completed")
...
def find_unzip_folder(data_source, zip_file_path = None):
    ...
    # Extract the contents of the zip file to a new folder
        ...
        log.info(f"No {data_source} file to unzip")
        return False
...
def clean_rename_move_folder(export_folder, download_folder, folder_name, new_folder_name):
    ...
    if not os.path.exists(folder_path):
        log.info(f"Folder {folder_path} does not exist in Downloads")
        return False
    old_export_folder = os.path.join(export_folder, new_folder_name)
    log.debug(f"Removing old export folder: {old_export_folder}")
...
def record_successful_run(source_name: str, pipeline_type: str = 'active'):
    ...
    try:
        ...
        if source_name in df['source_name'].values:
            ...
            log.progress(f"Updated tracking for {source_name}: {current_timestamp}")
        else:
            # Add new record
            new_row = {
            ...
            log.progress(f"Added new tracking for {source_name}: {current_timestamp}")

        # Save updated tracking file
        df.to_csv(tracking_file, sep=',', index=False, encoding='utf-8')

    except Exception as e:
        log.warning(f"Could not update tracking file: {e}")
...
def get_last_successful_runs():
    ...
    try:
        if os.path.exists(tracking_file):
            ...
            log.info(f"Tracking file not found: {tracking_file}")
            return pd.DataFrame(columns=['source_name', 'last_successful_run', 'status', 'pipeline_type'])
    except Exception as e:
        log.warning(f"Error reading tracking file: {e}")
        return pd.DataFrame(columns=['source_name', 'last_successful_run', 'status', 'pipeline_type'])
...
def display_tracking_summary():
    ...
    if df.empty:
        log.info("No tracking data available")
        return

    log.subheader("DATA SOURCE TRACKING SUMMARY")
    ...
    for pipeline_type in ['active', 'coordination', 'legacy', 'inactive']:
        ...
        if not sources.empty:
            log.progress(f"{pipeline_type.upper()} SOURCES:")
            for _, row in sources.iterrows():
                ...
                log.summary(f"{status_icon} {row['source_name']:<20} | {last_run_str}")
