...
def get_authenticated_drive():
    ...
    if gauth.credentials is None:
        ...
        log.info("First time setup: Opening browser for authentication...")
        log.info("This will only happen once (or when tokens expire)")
        gauth.LocalWebserverAuth()

    elif gauth.access_token_expired:
        try:
            # Try to refresh the token automatically
            log.progress("Refreshing expired access token...")
            gauth.Refresh()
            ...
            log.info("Re-authenticating (refresh token expired)")
            gauth.LocalWebserverAuth()
    else:
        # Use existing valid credentials
        log.success("Using existing valid credentials")
        gauth.Authorize()
...
def check_credentials_status():
    ...
    if not os.path.exists(CREDENTIALS_FILE):
        log.error("No credentials file found. First authentication required.")
        return False
    ...
    try:
        ...
        if 'refresh_token' in creds and creds['refresh_token']:
            log.success("Refresh token present - long-term access enabled")

            # Check access token expiry (less important since it auto-refreshes)

            return True
        else:
            log.warning("No refresh token found - may need re-authentication soon")
            return False
...
def update_file(file_name):
    ...
    try:
        ...
        if not os.path.exists(file_name):
            log.error(f"Local file {file_name} does not exist")
            return False
        ...
        for file in fileList:
            if file['title'] == file_title:
                try:
                    ...
                    log.success(f"{file_name} updated in Google Drive")
                    return True
                except Exception as update_error:
                    # If update fails due to permissions, try to create new file
                    log.warning(f"Update failed for {file_name}, trying to create new file...")
                    log.info(f"Update error: {str(update_error)[:100]}")
                    break
        ...
        try:
            ...
            new_file.Upload()
            log.success(f"{file_name} created in Google Drive")
            return True
        except Exception as create_error:
            log.error(f"Failed to create {file_name}: {create_error}")
            return False

    except Exception as e:
        log.error(f"Error with {file_name}: {e}")
        return False
...
def update_drive(file_names):
    ...
    try:
                ...
                for file in fileList:
                    if file['title'] == file_title:
                        ...
                        try:
                            ...
                            log.warning(f"Update failed for {file_name}: {str(update_error)[:100]}")
                            break
        ...
        log.success(f"Uploaded {total_successful} file(s) ({len(successful_updates)} updated, {len(successful_creates)} created)")

        if failed_updates:
            for file in failed_updates:
                log.error(f"Upload failed: {file}")

        if successful_updates:
            for file in successful_updates:
                log.debug(f"Updated: {file}")

        if successful_creates:
            for file in successful_creates:
                log.debug(f"Created: {file}")

        return len(failed_updates) == 0

    except Exception as e:
        log.error(f"Critical error in drive update: {e}")
        return False
...
def test_drive_connection():
    ...
    try:
        log.progress("Testing Google Drive connection...")
        drive = get_authenticated_drive()

        # Try to list files in the folder
        fileList = drive.ListFile({'q': f"'{folder_id}' in parents and trashed=false"}).GetList()
        log.success(f"Drive connection OK ({len(fileList)} files in folder)")
