...
def expand_df(df, processed_file_path):
    ...
    if os.path.exists(processed_file_path):
        log.info(f"📁 Loading existing processed file: {processed_file_path}")
        ...
        log.debug(f"Found {df.shape[0]} new rows to expand")
    else:
        log.warning(f"No existing processed file found, processing all {df.shape[0]} rows")
...
def _legacy_screentime_before_sleep(df):
    ...
    """
    log.warning("This function is deprecated - sleep calculation moved to health_processing.py")
    return df
...
def download_offscreen_data():
    ...
    """
    try:
        log.progress("Starting Offscreen data download...")
        log.info("Instructions:")
        log.info(" 1. Open Offscreen app on your phone")
        log.info(" 2. Go to Settings > Export Data")
        log.info(" 3. Select 'Pickup.csv' for export")
        log.info(" 4. Share/export the file to your computer")
        log.info(" 5. Save the file to Downloads folder")
        log.info(" 6. File should be named with timestamp (e.g., '2024-01-15 12:30:45 000-Pickup.csv')")

        response = prompt_user_download_status("Offscreen")

        if response:
            log.success("Download confirmed")

        return response

    except Exception as e:
        log.error(f"Error during download process: {e}")
        return False
...
def move_offscreen_files():
    ...
    """
    try:
        log.info("📁 Moving Offscreen files...")
        ...
        if count_file == 0:
            log.error(f"No Offscreen export files found in {download_folder}")
            log.info(f"Looking for pattern: YYYY-MM-DD HH:MM:SS*.csv")
            return False

        log.success(f"Processed {count_file} Offscreen export file(s)")
        return True
...
def create_offscreen_file():
    ...
    Returns True if successful, False otherwise.
    """
    log.info("⚙️  Processing Offscreen data...")
    ...
    try:
        ...
        log.normal(f"Processing {len(df)} pickup records...")

        # Expand dataframe (one row per minute)
        log.progress("Expanding data to minute-level granularity...")
        ...
        log.success(f"Saving processed data to {output_file}...")
        df.to_csv(output_file, sep='|', index=False, encoding='utf-8')

        log.normal_success(f"Processed {len(df)} records — {df['screen_time'].sum() / 3600:.1f}h screen time, {int(df['pickups'].sum())} pickups")
        log.progress(f"Data range: {df['date'].min()} to {df['date'].max()}")

        return True

    except Exception as e:
        log.error(f"Error processing Offscreen data: {e}")
        traceback.print_exc()
        return False
...
def full_offscreen_pipeline(auto_full=False, auto_process_only=False):
    ...
    """
    log.info("\n" + "="*60)
    log.progress("OFFSCREEN (SCREEN TIME) SOURCE PROCESSOR")
    log.info("="*60)
    ...
    log.info("\n" + "="*60)
    if success:
        log.success("Offscreen source processing completed successfully!")
        log.info("📁 Output: files/source_processed_files/offscreen/offscreen_processed.csv")
        # Record successful run
        record_successful_run('source_offscreen', 'active')
    else:
        log.error("Offscreen source processing failed")
    log.info("="*60)
...
if __name__ == "__main__":
    ...
    log.progress("Offscreen Source Processing Tool")
    log.info("This tool downloads and processes Offscreen screentime data.")
