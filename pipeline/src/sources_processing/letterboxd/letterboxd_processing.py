...
def load_tmdb_cache():
    ...
    try:
        ...
        log.warning(f"Error loading TMDB cache: {e}")
    return {}
...
def save_to_tmdb_cache(movie_key, movie_data):
    ...
    try:
        ...
        log.warning(f"Error saving to TMDB cache: {e}")
        return False
...
def get_tmdb_movie_info(title, release_year):
    ...
    if movie_key in cache:
        ...
        if (cached_data.get('poster_url') != 'No poster found' and
            ...
            log.progress(f"Using cached data for {title} ({release_year})")
            return cached_data

    # Get TMDB API key from environment
    api_key = os.environ.get('TMDB_Key')
    if not api_key:
        log.info("TMDB_API_KEY not found in environment variables")
        return _get_default_movie_data()

    try:
        log.progress(f"Fetching comprehensive data for: {title} ({release_year})")
        ...
        if response.status_code != 200:
            log.error(f"TMDB search API error {response.status_code} for {title} ({release_year})")
            return _get_default_movie_data()

        data = response.json()

        if not data['results'] or len(data['results']) == 0:
            log.error(f"No TMDB results found for {title} ({release_year})")
            return _get_default_movie_data()
        ...
        if details_response.status_code != 200:
            log.error(f"TMDB details API error {details_response.status_code} for {title} ({release_year})")
            return _get_default_movie_data()
        ...
        log.success(f"Found comprehensive data for {title} ({release_year})")
        log.info(f"Director: {movie_data.get('director', 'Unknown')}, Runtime: {movie_data.get('runtime', 'N/A')} min, Rating: {movie_data.get('vote_average', 'N/A')}")

        # Normal-level per-movie confirmation
        has_poster = movie_data.get('poster_url') != 'No poster found'
        has_data = movie_data.get('genres') != 'Unknown'
        if has_poster and has_data:
            log.normal_success(f"{title} ({release_year}) — poster: yes, data: yes")
        else:
            poster_status = "yes" if has_poster else "no"
            data_status = "yes" if has_data else "no"
            log.normal(f"  {title} ({release_year}) — poster: {poster_status}, data: {data_status}")
        ...
        log.error(f"Error fetching data for {title} ({release_year}): {str(e)}")
        return _get_default_movie_data()
...
def download_letterboxd_data():
    ...
    """
    log.progress("Starting Letterboxd data download...")
    ...
    log.info(" 4. The file will be named like 'letterboxd-username-YYYY-MM-DD-HH-MM-utc.zip'")
...
def move_letterboxd_files():
    ...
    if move_success:
        log.success("Successfully moved Letterboxd files to exports folder")
    else:
        log.error("Failed to move Letterboxd files")
...
def create_letterboxd_file():
    ...
    """
    log.info("⚙️  Processing Letterboxd source data...")
    ...
    try:
        ...
        if not os.path.exists(path_watched):
            log.error(f"Watched file not found: {path_watched}")
            return False

        if not os.path.exists(path_ratings):
            log.error(f"Ratings file not found: {path_ratings}")
            return False

        if not os.path.exists(path_diary):
            log.error(f"Diary file not found: {path_diary}")
            return False

        # Merge watched, ratings, and diary data
        log.progress("Reading and merging watched, ratings, and diary data...")
        df = get_watched_rating(path_watched, path_ratings, path_diary)

        # Add comprehensive movie data from TMDB
        log.progress("Adding comprehensive movie information from TMDB...")
        log.progress(f"Processing {len(df)} movies for TMDB data...")

        # Get unique movies to avoid duplicate API calls
        unique_movies = df[['Name', 'Year']].drop_duplicates()
        log.success(f"Found {len(unique_movies)} unique movies")

        # Pre-check cache to determine which movies need TMDB fetching
        cache = load_tmdb_cache()
        cached_count = 0
        new_movies_list = []
        for _, row in unique_movies.iterrows():
            movie_key = f"{row['Name']}_{row['Year']}"
            cached_data = cache.get(movie_key, {})
            if (cached_data.get('poster_url') != 'No poster found' and
                cached_data.get('genres') != 'Unknown'):
                cached_count += 1
            else:
                new_movies_list.append((row['Name'], row['Year']))

        log.normal(f"TMDB enrichment: {cached_count} cached, {len(new_movies_list)} new movies to fetch")
        if new_movies_list:
            log.normal("New movies to fetch from TMDB:")
            for movie_name, movie_year in new_movies_list:
                log.normal(f"  • {movie_name} ({movie_year})")
        ...
        for idx, row in unique_movies.iterrows():
            ...
            if (idx + 1) % 10 == 0:
                log.info(f"Processed {idx + 1}/{len(unique_movies)} unique movies")
        ...
        # Save to CSV
        log.success(f"Saving source processed data to {output_path}...")
        df.to_csv(output_path, sep='|', index=False, encoding='utf-8')

        log.success(f"\n Processing complete!")
        log.progress(f"Processed {len(df)} movie entries")
        ...
        # Normal-level end summary
        log.normal(f"Letterboxd processing summary: {len(df)} entries, {len(movie_data_dict)} unique movies")
        log.normal(f"  Posters: {movies_with_posters}/{len(movie_data_dict)} | Directors: {movies_with_directors}/{len(movie_data_dict)} | Runtime: {movies_with_runtime}/{len(movie_data_dict)}")
        if len(new_movies_list) > 0:
            log.normal_success(f"Fetched {len(new_movies_list)} new movies from TMDB, {cached_count} served from cache")
        else:
            log.normal_success(f"All {cached_count} movies served from cache — no TMDB calls needed")

        log.progress(f"Found posters for {movies_with_posters}/{len(movie_data_dict)} unique movies")
        log.info(f"Found directors for {movies_with_directors}/{len(movie_data_dict)} unique movies")
        log.info(f"⏱️  Found runtime for {movies_with_runtime}/{len(movie_data_dict)} unique movies")
        log.info(f"🎥 Found trailers for {movies_with_trailer}/{len(movie_data_dict)} unique movies")
        ...
        if not movies_with_data.empty:
            ...
            for _, movie in movies_with_data.iterrows():
                log.info(f"• {movie['name']} ({movie['year']})")
                log.info(f" Director: {movie['director']}")
                log.info(f" Runtime: {movie['runtime']} min" if movie['runtime'] else "   Runtime: N/A")
                log.info(f" Cast: {movie['cast'][:80]}..." if len(str(movie['cast'])) > 80 else f"  Cast: {movie['cast']}")
                log.info(f" Certification: {movie['certification']}")
                log.info(f" IMDb ID: {movie['imdb_id']}")

        return True

    except Exception as e:
        log.error(f"Error processing Letterboxd data: {e}")
        return False
...
def full_letterboxd_pipeline(auto_full=False, auto_process_only=False):
    ...
    """
    log.info("\n" + "="*60)
    log.progress("LETTERBOXD SOURCE DATA PIPELINE")
    log.info("="*60)
    ...
    log.info("\n" + "="*60)
    if success:
        log.success("Letterboxd source pipeline completed successfully!")
        log.info("Note: To upload to Drive, run the Movies topic pipeline.")
        # Record successful run
        record_successful_run('source_letterboxd', 'active')
    else:
        log.error("Letterboxd source pipeline failed")
    log.info("="*60)
...
def process_letterboxd_export(upload="Y"):
    ...
    """
    log.warning("process_letterboxd_export() is deprecated.")
...
if __name__ == "__main__":
    ...
    log.progress("Letterboxd Source Processing Tool")
    log.info("This tool processes Letterboxd exports into source data files.")
    log.info("For website generation and upload, use the Movies topic coordinator.")
