...
def load_location_data():
    ...
    if not os.path.exists(location_path):
        ...
        log.info(" Run the location pipeline first.")
        return None

    log.info(f"📍 Loading location data from {location_path}...")
    ...
    log.success(f"Loaded {len(df):,} location records")
    log.info(f"Date range: {df['timestamp'].min().date()} to {df['timestamp'].max().date()}")
...
def load_existing_weather_data():
    ...
    if os.path.exists(output_path):
        ...
        log.info(f"Found {len(df):,} existing weather records")
        return df

    log.info("No existing weather data found, starting fresh")
    return pd.DataFrame()
...
def fetch_weather_for_location(city, latitude, longitude, start_date, end_date):
    ...
    if not stations:
        log.warning(f"No stations found near {city}")
        return None
    ...
    log.warning(f"No weather data available for {city}")
    return None
...
def create_meteostat_file():
    ...
    if len(missing_df) == 0:
        log.normal_success("All weather data already up to date!")
        return True

    log.normal(f"Need weather data for {len(missing_df):,} location records")

    # Group into date ranges by location
    date_ranges = get_location_date_ranges(missing_df)
    log.info(f"📅 Fetching weather for {len(date_ranges)} location/date range combinations...")

    # Fetch weather for each range
    all_weather = []
    for i, row in date_ranges.iterrows():
        log.info(f"[{i+1}/{len(date_ranges)}] {row['city']}: {row['start_date'].date()} to {row['end_date'].date()}")
    ...
    if not all_weather:
        log.warning("No new weather data fetched")
    ...
    log.normal_success(f"Fetched {len(new_weather_df):,} new weather records")
    ...
    log.normal_success(f"Saved {len(combined_df):,} total weather records")
    return True
...
def full_meteostat_pipeline(auto_full=False, auto_process_only=False):
    ...
    """
    log.info("\n" + "="*60)
    log.info("🌤️  METEOSTAT SOURCE PIPELINE")
    log.info("="*60)

    # Check API credentials
    if not api_key or not api_host:
        log.error("Meteostat API credentials not found in .env file")
        log.info(" Required: Meteostats_API_KEY, Meteostats_API_HOST")
        return False

    log.progress("\n Fetching weather data based on location history...")
    log.info(" Input: Location pipeline output (location_processed.csv)")
    ...
    try:
        ...
        log.info("\n" + "="*60)
        if success:
            log.success("Meteostat source pipeline completed!")
            log.info(f"📁 Output: files/source_processed_files/meteostat/meteostat_processed.csv")
        else:
            log.error("Meteostat source pipeline failed")
        log.info("="*60)
...
if __name__ == "__main__":
    log.info("🌤️  Meteostat Source Processing Tool")
    log.info("This tool fetches weather data from Meteostat API")
    log.info("based on your location history from the location pipeline.")
