...
def clean_geocoding_cache():
    ...
    """
    log.progress("Cleaning geocoding cache...")
    ...
    if initial_count == 0:
        log.info(" Cache is empty, nothing to clean")
        return (0, 0, 0)
    ...
    log.success(f"Cache cleaning complete:")
    log.info(f"• Removed {removed_count} incomplete entries")
    log.info(f"• Before: {initial_count} entries")
    log.info(f"• After: {final_count} entries")
    log.info(f"• These will be re-geocoded on next pipeline run")
...
def load_place_name_mappings(coordinate_mapping: Dict = None) -> Dict:
    ...
    try:
        ...
        log.warning(f"Error loading place name mappings: {e}")
        return {}
    ...
    if coordinate_mapping:
        ...
        if migrations_needed:
            ...
            log.progress(f"Migrating {len(mappings)} place name mappings to canonical coordinates...")
...
def save_place_name_mappings(mappings: Dict):
    ...
    try:
        ...
        log.success(f"Saved {len(mappings)} place name mappings")
    except Exception as e:
        log.error(f"Error saving place name mappings: {e}")
...
def apply_place_name_mappings(geocoding_cache: Dict, mappings: Dict) -> Dict:
    ...
    if updated_count > 0:
        log.info(f"📍 Applied {updated_count} custom place names")
...
def consolidate_geocoding_cache(geocoding_cache: Dict) -> tuple:
    ...
    if consolidated_count > 0:
        log.info(f"🔗 Consolidated {consolidated_count} addresses with multiple coordinates")
...
def download_google_data():
    ...
    """
    log.progress("Starting Google Maps Timeline data download...")

    log.info("Instructions for iPhone export:")
    log.info(" 1. Open Google Maps app on your iPhone")
    log.info(" 2. Tap your profile picture (top right)")
    log.info(" 3. Go to 'Your timeline'")
    log.info(" 4. Tap the settings/menu button")
    log.info(" 5. Select 'Export timeline data'")
    log.info(" 6. Choose your date range")
    log.info(" 7. Download the JSON file")
    log.info(" 8. AirDrop the 'location-history.json' file to your Mac Downloads folder")
...
def move_google_files():
    ...
    """
    log.info("📁 Moving Google Maps files...")
    ...
    if success:
        log.success("Successfully moved Google Maps export to exports folder")
    else:
        log.error("Failed to move Google Maps files")
...
def create_hourly_timezone_records(location_data: List[Dict]) -> List[Dict]:
    ...
    """
    log.progress("Creating hourly timezone and location records...")
    ...
    for item in location_data:
        ...
        try:
            ...
            log.warning(f"Error processing timeline item: {e}")
            continue

    if not timeline_points:
        log.error("No valid timeline points found")
        return []

    # Sort by start time
    timeline_points.sort(key=lambda x: x['start_time'])

    log.progress(f"Found {len(timeline_points)} valid timeline points")

    # Find the overall date range
    min_time = timeline_points[0]['start_time']
    max_time = timeline_points[-1]['end_time']

    log.info(f"📅 Date range: {min_time.date()} to {max_time.date()}")

    # OPTIMIZATION 1: Load existing geocoding cache and geocode only new coordinates
    log.progress("Loading geocoding cache and processing coordinates...")
    geocoding_cache = load_geocoding_cache()
    ...
    # NORMAL log: geocoding scope for hourly records
    log.normal(f"Hourly geocoding: {len(coordinates_to_geocode)} new locations, {len(unique_coordinates) - len(coordinates_to_geocode)} cached")

    if coordinates_to_geocode:
        log.info(f"📍 Need to geocode {len(coordinates_to_geocode)} new locations (already have {len(geocoding_cache)} cached)")

        for idx, coordinates in enumerate(coordinates_to_geocode, 1):
            if idx % 10 == 0:
                log.info(f"Geocoded {idx}/{len(coordinates_to_geocode)} new locations...")
            ...
            if idx % 50 == 0:
                save_geocoding_cache(geocoding_cache)
                log.success(f"Saved cache checkpoint at {idx} locations")

        # Final save
        save_geocoding_cache(geocoding_cache)
        log.success(f"Geocoded {len(coordinates_to_geocode)} new locations")
    else:
        log.success(f"All {len(unique_coordinates)} locations already cached - no geocoding needed!")

    # OPTIMIZATION 2: Build index for fast timeline lookup
    log.progress("Building timeline index for fast lookup...")
    ...
    log.success(f"Indexed {len(timeline_index)} hours")

    # OPTIMIZATION 3: Generate hourly records with O(1) lookup
    log.info("Generating hourly records...")
    hourly_records = []
    ...
    while current_hour <= max_time:
        ...
        if hours_processed % 500 == 0 and hours_processed > 0:
            log.info(f"Processed {hours_processed}/{total_hours} hours...")
...
def create_minute_location_records(location_data: List[Dict]) -> List[Dict]:
    ...
    """
    log.progress("Creating minute-by-minute location records (visits + activities)...")
    ...
    log.progress(f"Found {len(visit_records)} visits, {len(activity_records)} activities")

    # Step 2: Geocode visit and activity start coordinates
    log.progress("Loading geocoding cache and processing locations...")
    geocoding_cache = load_geocoding_cache()
    ...
    # NORMAL log: geocoding scope for minute records
    log.normal(f"Minute geocoding: {len(coordinates_to_geocode)} new locations, {len(unique_coordinates) - len(coordinates_to_geocode)} cached")
    ...
    if coordinates_to_geocode:
        ...
        for idx, coordinates in enumerate(coordinates_to_geocode, 1):
            if idx % 10 == 0 or idx == len(coordinates_to_geocode):
                log.info(f"Geocoding {idx}/{len(coordinates_to_geocode)} locations...")
        ...
        log.success(f"All {len(unique_coordinates)} locations already cached!")
    ...
    log.info("Generating minute-by-minute records from visits and activities...")

    # Find overall date range
    if not visit_records and not activity_records:
        log.error("No visit or activity records found")
        return []
    ...
    log.success(f"Created {len(sorted_records)} minute records from {len(visit_records)} visits + {len(activity_records)} activities")
    ...
    log.progress(f"Location statistics:")
    log.info(f"• Total minutes: {len(sorted_records)}")
    log.info(f"• Visit minutes: {visit_minutes} ({(visit_minutes/len(sorted_records)*100):.1f}%)")
    log.info(f"• Activity minutes: {activity_minutes} ({(activity_minutes/len(sorted_records)*100):.1f}%)")
    log.info(f"• Minutes at home: {home_minutes} ({(home_minutes/len(sorted_records)*100):.1f}%)")
    log.info(f"• Unique places visited: {unique_places}")

    if activity_types:
        log.progress(f"Activity type breakdown:")
        ...
        for activity_type, count in sorted_activities[:5]:  # Show top 5
            ...
            log.info(f"• {activity_type}: {count} minutes ({percentage:.1f}%)")
...
def create_google_maps_file():
    ...
    """
    log.info("\n" + "="*70)
    log.progress("GOOGLE MAPS TIMELINE PROCESSING")
    log.info("="*70)
    ...
    try:
        ...
        if not os.path.exists(input_path):
            log.error(f"Google Maps file not found: {input_path}")
            return False
        ...
        log.success(f"Loaded {len(location_data)} timeline items")

        # ========================================================================
        # PART 1: Create hourly records (for timezone correction)
        # ========================================================================
        log.info("\n" + "="*70)
        log.info("PART 1: HOURLY LOCATION DATA (for timezone correction)")
        log.info("="*70)
        ...
        log.success(f"Saving hourly data to {hourly_output_path}...")
        df_hourly.to_csv(hourly_output_path, sep='|', index=False, encoding='utf-8')

        log.success(f"Successfully processed hourly Google Maps data!")
        log.progress(f"Created {len(df_hourly)} hourly location records")
        log.info(f"📅 Date range: {df_hourly['timestamp'].min()} to {df_hourly['timestamp'].max()}")
        log.progress(f"Countries visited: {df_hourly['country'].nunique()}")
        log.info(f"🏙️  Cities visited: {df_hourly['city'].nunique()}")
        log.info(f"🏠 Hours at home: {df_hourly['is_home'].sum()}")
        ...
        # ========================================================================
        log.info("\n" + "="*70)
        log.info("PART 2: MINUTE-LEVEL LOCATION DATA (detailed tracking)")
        log.info("="*70)

        minute_records = create_minute_location_records(location_data)

        if not minute_records:
            log.error("No minute records created")
            return False
        ...
        # Save to CSV with pipe separator and UTF-8 encoding
        log.success(f"Saving minute data to {minute_output_path}...")
        df_minute.to_csv(minute_output_path, sep='|', index=False, encoding='utf-8')

        log.success(f"Successfully processed minute-level Google Maps data!")
        log.progress(f"Created {len(df_minute)} minute location records")
        log.info(f"📅 Date range: {df_minute['timestamp'].min()} to {df_minute['timestamp'].max()}")
        log.progress(f"Countries visited: {df_minute['country'].nunique()}")
        log.info(f"🏙️  Cities visited: {df_minute['city'].nunique()}")
        log.info(f"📍 Unique places: {df_minute['place_name'].nunique()}")
        log.info(f"🏠 Minutes at home: {df_minute['is_home'].sum()}")
        ...
        for _, row in sample_df.iterrows():
            ...
            log.info(f"{icon} {row['timestamp'][:16]} | {place}{activity} | {status}")

        # ========================================================================
        # SUMMARY
        # ========================================================================
        log.info("\n" + "="*70)
        log.success("GOOGLE MAPS PROCESSING COMPLETE!")
        log.info("="*70)
        log.info(f"📄 Hourly file: {hourly_output_path}")
        log.info(f"• {len(df_hourly)} records | {len(df_hourly.columns)} columns")
        log.info(f"📄 Minute file: {minute_output_path}")
        log.info(f"• {len(df_minute)} records | {len(df_minute.columns)} columns")
        log.info("="*70)

        # NORMAL log: end summary
        log.normal_success(f"Google Maps: {len(df_hourly):,} hourly + {len(df_minute):,} minute records processed")

        return True

    except Exception as e:
        log.error(f"Error processing Google Maps data: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def identify_frequent_places():
    ...
    """
    log.info("\n" + "="*70)
    log.info("📍 IDENTIFY AND NAME FREQUENT PLACES")
    log.info("="*70)
    ...
    if not os.path.exists(minute_file):
        log.error(f"Minute-level file not found: {minute_file}")
        log.info(" Run option 2 first to process existing data.")
        return False

    try:
        df = pd.read_csv(minute_file, sep='|', encoding='utf-8')
        log.success(f"Loaded {len(df):,} minute records")
    except Exception as e:
        log.error(f"Error loading minute file: {e}")
        return False
    ...
    log.info(f"📍 Filtered to {len(visits_only):,} visit records (excluding transit)")

    if len(visits_only) == 0:
        log.error("No visit records found")
        return False

    # Filter out records without a proper address
    visits_only = visits_only[visits_only['address'].notna() & (visits_only['address'] != '')]
    log.info(f"📍 {len(visits_only):,} records have actual addresses")

    if len(visits_only) == 0:
        log.error("No visit records with addresses found")
        return False
    ...
    log.progress(f"Found {len(frequent_places)} places visited on 3+ different dates")

    # Load existing mappings
    existing_mappings = load_place_name_mappings()
    log.info(f"{len(existing_mappings)} existing custom place names")
    ...
    if not unnamed_places:
        log.success("All frequent places already have custom names!")
        return True
    ...
    for i, place in enumerate(unnamed_places, 1):
        ...
        log.info(f"Current name: {current_name}")
        log.info(f"Address: {place['address']}")
        log.info(f"City: {place['city']}")
        log.info(f"Dates: {dates_str}")
        ...
        if new_name:
            ...
            named_count += 1
            log.success(f"Named as: {new_name}")
...
def update_place_name():
    ...
    """
    log.info("\n" + "="*70)
    log.info("MANUALLY UPDATE PLACE NAME")
    log.info("="*70)

    # Load geocoding cache for searching
    geocoding_cache = load_geocoding_cache()
    if not geocoding_cache:
        log.error("Geocoding cache is empty. Run processing first.")
        return False

    log.success(f"Loaded {len(geocoding_cache)} cached locations")
    ...
    while True:
        ...
        if not matches:
            log.error(f"No locations found containing '{search_term}'")
            continue
        ...
        for i, loc in enumerate(matches[:20], 1):  # Limit to 20 results
            ...
            log.info(f"{i:2}. {display_name}{custom_indicator}")
            log.info(f" Address: {address}")
            log.blank()

        if len(matches) > 20:
            log.info(f"... and {len(matches) - 20} more. Refine your search to see others.")

        # Get user selection
        try:
            choice = int(input(f"Select location (1-{min(len(matches), 20)}): "))
            if choice < 1 or choice > min(len(matches), 20):
                log.error(f"Invalid choice. Please select 1-{min(len(matches), 20)}")
                continue
        except ValueError:
        ...
        log.info(f"Current name: {current_display}")
        log.info(f"Coordinates: {selected['coordinates']}")

        # Get new name
        new_name = input("\nEnter new place name: ").strip()
        if not new_name:
            log.error("No name provided, skipping")
            continue
        ...
        log.success(f"Updated place name to: {new_name}")
    ...
    if not updated_places:
        log.error("No places were updated")
        return False
...
def full_google_maps_pipeline(auto_full=False, auto_process_only=False):
    """
    ...
    """
    log.info("\n" + "="*70)
    log.progress("GOOGLE MAPS SOURCE PROCESSOR PIPELINE")
    log.info("="*70)
    ...
    if auto_process_only:
        ...
        log.prompt("3. Identify and name frequent places")
        log.prompt("4. Manually update a place name")
    ...
    log.info("\n" + "="*70)
    if success:
        log.success("Google Maps source processor completed successfully!")
        log.progress("Outputs:")
        log.info(" • Hourly: files/source_processed_files/google_maps/google_maps_processed.csv")
        log.info(" • Minute: files/source_processed_files/google_maps/google_maps_minute_processed.csv")
...
if __name__ == "__main__":
    ...
    log.progress("Google Maps Timeline Source Processor")
    log.info("This processor handles Google Maps Timeline data.")
