...
class USDANutritionScorer:
    ...
    def _load_cache(self) -> Dict:
        ...
        try:
            ...
            log.warning(f"Error loading nutrition cache: {e}")
        return {}
    ...
    def _save_cache(self):
        ...
        try:
            ...
            log.warning(f"Error saving nutrition cache: {e}")
    ...
    def search_usda_food(self, ingredient_name: str) -> Optional[Dict]:
        ...
        try:
            ...
            if response.status_code == 429:  # Rate limited
                log.warning(f"Rate limited for '{ingredient_name}', using fallback")
                return None
            ...
            log.warning(f"API error for '{ingredient_name}': {e}")
        except Exception as e:
            log.warning(f"Error processing '{ingredient_name}': {e}")
...
def extract_unique_ingredients_from_dataframe(df) -> List[str]:
    ...
    for _, row in meal_rows.iterrows():
        ...
        try:
            ...
            log.warning(f"Error parsing ingredients from: {ingredients_str}")
            continue
    ...
    if os.path.exists(database_file):
        try:
            with open(database_file, 'r') as f:
                data = json.load(f)
            log.progress(f"Loaded {len(data)} ingredient scores from database")
            return data
        except Exception as e:
            log.warning(f"Error loading ingredient database: {e}")
    
    log.progress("No ingredient database found - creating new one")
    return {}
...
def save_ingredient_scores_database(ingredient_scores: Dict[str, Tuple[float, str]]):
    ...
    try:
        ...
        log.success(f"Saved {len(ingredient_scores)} ingredient scores to database")
    except Exception as e:
        log.error(f"Error saving ingredient database: {e}")
...
def score_all_ingredients_efficient(unique_ingredients: List[str], api_key: Optional[str] = None) -> Dict[str, Tuple[float, str]]:
    ...
    Returns:
        Dict[str, Tuple[float, str]]: Dictionary mapping ingredient -> (score, source)
    """
    log.progress(f"Processing {len(unique_ingredients)} unique ingredients...")
    ...
    log.progress(f"Status: {already_scored} already scored, {len(need_scoring)} need scoring")
    
    if len(need_scoring) == 0:
        log.normal_success(f"All {len(unique_ingredients)} ingredients already scored in database")
        log.progress("All ingredients already in database - no API calls needed!")
    else:
        log.normal(f"Scoring {len(need_scoring)} new ingredient(s)...")
        log.progress(f"Scoring {len(need_scoring)} new ingredients...")
        ...
        for i, ingredient in enumerate(need_scoring):
            ...
            if source == 'usda':
                usda_count += 1
                log.normal(f"  + {ingredient} -> score {score} (USDA match)")
            elif source == 'fallback':
                fallback_count += 1
                log.normal(f"  + {ingredient} -> score {score} (fallback database)")
            elif source == 'category':
                category_count += 1
                log.normal(f"  + {ingredient} -> score {score} (category match)")
            else:  # default
                default_count += 1
                log.normal(f"  + {ingredient} -> score {score} (default — needs manual scoring)")

            # Progress indicator
            if (i + 1) % 10 == 0 or i == len(need_scoring) - 1:
                log.info(f"Processed {i + 1}/{len(need_scoring)} new ingredients...")

        # Save updated database
        save_ingredient_scores_database(ingredient_scores)

        # Normal-level summary of new ingredient scoring
        total_new = len(need_scoring)
        log.normal_success(f"Scored {total_new} new ingredient(s): {usda_count} USDA, {fallback_count} fallback, {category_count} category, {default_count} default")

        # Report on new ingredients
        log.progress(f"\n New Ingredient Results:")
        log.progress(f"USDA: {usda_count}/{total_new} ({usda_count/total_new*100:.1f}%)")
        log.info(f"🌏 Fallback: {fallback_count}/{total_new} ({fallback_count/total_new*100:.1f}%)")
        log.debug(f"️ Category: {category_count}/{total_new} ({category_count/total_new*100:.1f}%)")
        log.info(f" Default: {default_count}/{total_new} ({default_count/total_new*100:.1f}%)")
...
def flag_default_scores(ingredient_scores: Dict[str, Tuple[float, str]], current_ingredients: List[str]):
    ...
    if current_defaults:
        ...
        for item in current_defaults:
            log.info(f" '{item['ingredient']}' {item['score']} (needs manual scoring)")
        ...
        if os.path.exists(flagged_file):
            try:
                ...
                log.warning(f"Error loading flagged file: {e}")
        ...
        try:
            ...
            log.info(f"Updated flagged ingredients file: {flagged_file}")
            log.info(f"Tip: Review this file to add missing ingredients to the fallback database")
        except Exception as e:
            log.error(f"Error saving flagged file: {e}")
    else:
...
def calculate_meal_scores_from_ingredients(df, ingredient_scores: Dict[str, Tuple[float, str]]) -> pd.DataFrame:
    ...
    log.success(f"Meal scoring complete: {scored_count}/{total_meals} meals scored ({scored_count/total_meals*100:.1f}%)")
    return df
...
if __name__ == "__main__":
    ...
    log.progress("Testing USDA Nutrition Scorer")
    
    for ingredient in test_ingredients:
        score, source = scorer.get_ingredient_score(ingredient)
        log.info(f"{ingredient:20} | Score: {score:4.1f} | Source: {source}")