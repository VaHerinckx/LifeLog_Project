...
class USDDrinkScorer:
    ...
    def _load_cache(self) -> Dict:
        ...
        try:
            ...
            log.warning(f"Error loading drinks cache: {e}")
        return {}
    ...
    def _save_cache(self):
        ...
        try:
            ...
            log.warning(f"Error saving drinks cache: {e}")
        ...
        try:
            ...
            for search_term in search_terms:
                ...
                if response.status_code == 429:  # Rate limited
                    log.warning(f"Rate limited for '{drink_name}', using fallback")
                    return None
            ...
            log.warning(f"API error for '{drink_name}': {e}")
        except Exception as e:
            log.warning(f"Error processing '{drink_name}': {e}")
...
def extract_unique_drinks_from_dataframe(df) -> List[str]:
    ...
    if not drinks_list_col:
        log.warning("No 'drinks_list'column found in dataframe")
        log.debug(f"Available columns: {list(df.columns)}")
        return []
    
    log.progress(f"Found drinks column: {drinks_list_col}")
    ...
    for _, row in drink_rows.iterrows():
        ...
        try:
            float(drinks_str)
            log.warning(f"Skipping numeric value: {drinks_str}")
            continue
        ...
        try:
            ...
            log.warning(f"Error parsing drinks from: {drinks_str}")
            continue
    ...
    if os.path.exists(database_file):
        try:
            with open(database_file, 'r') as f:
                data = json.load(f)
            log.info(f"🍹 Loaded {len(data)} drink scores from database")
            return data
        except Exception as e:
            log.warning(f"Error loading drink database: {e}")
    
    log.info("🍹 No drink database found - creating new one")
    return {}
...
def save_drink_scores_database(drink_scores: Dict[str, Tuple[float, str, str]]):
    ...
    try:
        ...
        log.success(f"Saved {len(drink_scores)} drink scores to database")
    except Exception as e:
        log.error(f"Error saving drink database: {e}")
...
def score_all_drinks_efficient(unique_drinks: List[str], api_key: Optional[str] = None) -> Dict[str, Tuple[float, str, str]]:
    ...
    Returns:
        Dict[str, Tuple[float, str, str]]: Dictionary mapping drink -> (score, category, source)
    """
    log.info(f"🍹 Processing {len(unique_drinks)} unique drinks...")
    ...
    if len(need_scoring) == 0:
        log.normal_success(f"All {len(unique_drinks)} drinks already scored in database")
        log.progress("All drinks already in database - no API calls needed!")
    else:
        log.normal(f"Scoring {len(need_scoring)} new drink(s)...")
        log.progress(f"Scoring {len(need_scoring)} new drinks...")
        ...
        for i, drink in enumerate(need_scoring):
            ...
            if source == 'usda':
                usda_count += 1
                log.normal(f"  + {drink} -> score {score}, {category} (USDA match)")
            elif source == 'fallback':
                fallback_count += 1
                log.normal(f"  + {drink} -> score {score}, {category} (fallback database)")
            elif source == 'category':
                category_count += 1
                log.normal(f"  + {drink} -> score {score}, {category} (category match)")
            else:  # default
                default_count += 1
                log.normal(f"  + {drink} -> score {score}, {category} (default — needs manual scoring)")

            # Progress indicator
            if (i + 1) % 5 == 0 or i == len(need_scoring) - 1:
                log.info(f"Processed {i + 1}/{len(need_scoring)} new drinks...")

        # Save updated database
        save_drink_scores_database(drink_scores)

        # Normal-level summary of new drink scoring
        total_new = len(need_scoring)
        log.normal_success(f"Scored {total_new} new drink(s): {usda_count} USDA, {fallback_count} fallback, {category_count} category, {default_count} default")
...
def flag_default_drinks(drink_scores: Dict[str, Tuple[float, str, str]], current_drinks: List[str]):
    ...
    if current_defaults:
        ...
        for item in current_defaults:
            log.info(f" '{item['drink']}' {item['score']} ({item['category']}) - needs manual scoring")
        ...
        try:
            ...
            log.info(f"Updated flagged drinks file: {flagged_file}")
            log.info(f"Tip: Review this file to add missing drinks to the fallback database")
        except Exception as e:
...
if __name__ == "__main__":
    ...
    log.info("🍹 Testing USDA Drink Scorer")
    
    for drink in test_drinks:
        score, category, source = scorer.get_drink_score(drink)
        log.info(f"{drink:15} | Score: {score:4.1f} | Category: {category:20} | Source: {source}")