...
def score_meal_with_usda(ingredient_list, quantity="medium", use_usda=False):
    ...
    try:
        ...
        log.warning(f"Error in USDA meal scoring: {e}")
        return None
...
def add_usda_meal_scoring_efficient(df, use_usda_scoring=False):
    ...
    if not use_usda_scoring:
        log.info("USDA meal scoring disabled (use_usda_scoring=False)")
        return df

    log.info("Starting efficient USDA-based meal scoring...")

    meal_rows = df[df['meal'].notna() & (df['meal'] != '')].copy()

    if len(meal_rows) == 0:
        log.info("No meal data found for USDA scoring")
        return df

    try:
        # Use imports from top of file
        # Phase 1: Extract unique ingredients
        log.progress(f"Found {len(meal_rows)} meal entries to analyze")
        unique_ingredients = extract_unique_ingredients_from_dataframe(df)
        log.progress(f"Extracted {len(unique_ingredients)} unique ingredients from dataset")
        ...
        log.error(f"Error in efficient USDA meal scoring: {e}")
        log.progress("Falling back to original method...")
...
def add_usda_meal_scoring_legacy(df, use_usda_scoring=False):
    ...
    log.info("Using legacy USDA meal scoring method...")
    ...
    for idx, row in meal_rows.iterrows():
        ...
        if (scored_count + 1) % 50 == 0:
            log.info(f"Processed {scored_count + 1}/{total_meals} meals...")

    log.success(f"Legacy USDA scoring complete: {scored_count}/{total_meals} meals scored ({scored_count/total_meals*100:.1f}%)")
...
def add_usda_drink_scoring_efficient(df, use_usda_scoring=True):
    ...
    """
    if not use_usda_scoring:
        log.info("USDA drink scoring disabled (use_usda_scoring=False)")
        return df

    log.info("🍹 Starting efficient USDA-based drink scoring...")

    # Check if we have the drinks_list column (created by extract_data_count)
    if 'drinks_list' not in df.columns:
        log.info("No 'drinks_list'column found for USDA drink scoring")
        return df
    ...
    try:
        ...
        if not unique_drinks:
            log.info("No drinks found in dataset")
            return df

        log.progress(f"Extracted {len(unique_drinks)} unique drinks from dataset")
        ...
        log.success(f"Drink scoring complete - {len(drink_scores)} drinks processed")

        return df

    except Exception as e:
        log.error(f"Error in efficient USDA drink scoring: {e}")
        log.progress("Continuing without drink scoring...")
        return df
...
def check_new_drinks(drinks):
    ...
    if len(new_drinks) == 0:
        log.success("No new drinks found")
    else:
        log.info(f"🍹 Found {len(new_drinks)} new drinks:")
        for drink in new_drinks[:10]:  # Show first 10
            log.info(f"• {drink}")
        if len(new_drinks) > 10:
            log.info(f"... and {len(new_drinks) - 10} more")
        log.info("Note: New drinks need to be manually categorized in the drinks database")
        log.info("  File: files/work_files/nutrilio_work_files/nutrilio_drinks_category.xlsx")
...
def drinks_category(drinks):
    ...
    if new_drinks_count == 0:
        return "OK"
    if new_drinks_count > 0:
        log.info(chat_gpt_prompt + new_drinks_list)
...
def display_meal_validation_report(invalid_meals_list):
    ...
    if not invalid_meals_list:
        log.success("All meal types are valid!")
        return

    log.info("\n" + "="*70)
    log.warning("MEAL VALIDATION REPORT")
    log.info("="*70)
    log.info(f"\nFound {len(invalid_meals_list)} meal(s) with invalid meal types.")
    log.info(f"Valid meal types: {', '.join(VALID_MEAL_TYPES)}")
    log.info("\nPlease add the correct meal type tag in the Nutrilio app for:")
    log.info("\n" + "-"*70)
    log.info(f"{'Date':<12} {'Time':<8} {'Invalid Meal Type':<40}")
    log.info("-"*70)
    ...
    for entry in sorted_meals:
        ...
        log.info(f"{date_str:<12} {time_str:<8} {meal_str:<40}")
...
def merge_dreams_data(nutrilio_df):
    ...
    if os.path.exists(dreams_path):
        ...
        log.success(f"Dreams data merged: {len(dreams_aggregated)} dates with dreams")
        if multiple_dreams_count > 0:
            log.info(f"{multiple_dreams_count} dates had multiple dreams (combined with comma)")
    else:
        log.warning(f"Dreams file not found: {dreams_path}")
        log.info(" Adding empty dreams column")
...
def rename_columns_for_health(df):
    ...
    if 'notes_about_today' in df.columns:
        ...
        log.info("✏️  Renamed: notes_about_today  notes_summary")
...
def create_optimized_nutrition_file(df):
    ...
    log.progress("Creating optimized nutrition file...")

    # Filter only meal entries (exclude non-meal data)
    meal_df = df[df['meal'].notna() & (df['meal'] != '')].copy()
    log.info(f"Filtered to {len(meal_df)} meal entries from {len(df)} total rows")
    ...
    log.success(f"Created optimized nutrition file: {output_path}")
    log.progress(f"Optimized file contains {len(meal_df)} meal entries")
    log.debug(f"Reduced from {len(nutrition_cols)} to {len(final_cols)} columns")

    return output_path

def create_nutrilio_files():
    log.normal("Processing Nutrilio export data...")
    ...
    log.info("Enabling efficient USDA-based meal scoring...")
    ...
    log.info("🍹 Enabling efficient USDA-based drink scoring...")
    df = add_usda_drink_scoring_efficient(df, use_usda_scoring=True)

    log.info("⚙️  Finalizing data processing...")
    ...
    log.progress("Generating Power BI files...")
    ...
    log.info(f"Found {len(daily_summary_rows)} daily summary rows")
    log.info(f"Found {len(meal_rows)} meal rows")

    # Aggregate daily summary rows by date (keep latest time)
    log.progress("Aggregating daily summary rows by date...")
    if len(daily_summary_rows) > 0:
        ...
        if len(duplicates) > 0:
            log.info(f"Consolidated {len(duplicates)} dates with multiple daily summary entries:")
            for date, count in duplicates.items():
                times = daily_summary_rows[daily_summary_rows['date'] == date]['time'].tolist()
                log.info(f"   {date}: {count} entries at times {times}")
    ...
    if len(meal_rows) > 0:
        ...
        if len(duplicate_meals) > 0:
            ...
            for (date, meal_type), count in duplicate_meals.items():
                ...
                log.info(f"   {date} - {meal_type}: {count} entries at times {times}")
            log.warning("Keeping all entries for investigation. This should not happen normally.")
        else:
            log.success("No duplicate meals found")

    # Combine aggregated daily summary rows and meal rows
    log.info("🔗 Combining daily summary and meal rows...")
    ...
    log.success(f"Final dataframe: {len(final_df)} rows")
    log.info(f"Daily summary rows: {len(final_df[final_df['daily_summary'] == 'yes'])}")
    log.info(f"Meal rows: {len(final_df[final_df['meal'].notna()])}")
    ...
    log.success(f"Health file: {len(health_df)} daily summary rows  {health_file_path}")
    ...
    log.success(f"Nutrition file: {len(nutrition_df)} meal rows  {nutrition_file_path}")

    # DEPRECATED: Keep old combined file for backward compatibility (will be removed in future)
    log.warning("\n Also saving legacy combined file for backward compatibility...")
    final_df.to_csv("files/source_processed_files/nutrilio/nutrilio_processed.csv", sep='|', index=False, encoding='utf-8')
    log.warning(f"Legacy file: {len(final_df)} rows  nutrilio_processed.csv (DEPRECATED)")
    ...
    log.normal_success(f"Nutrilio processing complete — {len(final_df)} rows ({len(final_df[final_df['daily_summary'] == 'yes'])} daily summaries, {len(final_df[final_df['meal'].notna()])} meals), {len(drive_list)} files generated")
    log.success(f"Processing complete! Generated {len(drive_list)} files.")
    return drive_list
...
def download_nutrilio_data():
    ...
    """
    log.info("Starting Nutrilio data download...")

    log.info("Instructions:")
    log.info(" 1. Open the Nutrilio app on your phone")
    log.info(" 2. Go to Settings  Export Data")
    log.info(" 3. Select 'Export to CSV'")
    log.info(" 4. Choose email or share the file")
    log.info(" 5. Save the file to your Downloads folder")
    log.info(f"6. Rename the file to: Nutrilio-export-{date.today().strftime('%Y-%m-%d')}.csv")
...
def move_nutrilio_files():
    ...
    """
    log.info("📁 Moving Nutrilio files...")
    ...
    if move_success:
        log.success("Successfully moved Nutrilio file to exports folder")
    else:
        log.error("Failed to move Nutrilio file")
...
def create_nutrilio_processed_files():
    ...
    Returns True if successful, False otherwise.
    """
    log.info("⚙️  Processing Nutrilio data...")
    ...
    try:
        ...
        if not os.path.exists(input_path):
            log.error(f"Nutrilio file not found: {input_path}")
            return False

        # Process the files using the existing function
        nutrilio_files = create_nutrilio_files()

        log.success(f"Successfully processed {len(nutrilio_files)} Nutrilio files")
        log.progress("Generated files:")
        ...
        log.error(f"Error processing Nutrilio data: {e}")
        return False
...
def full_nutrilio_pipeline(auto_full=False, auto_process_only=False):
    ...
    """
    log.info("\n" + "="*60)
    log.info("NUTRILIO SOURCE PROCESSOR")
    log.info("="*60)
    ...
    log.info("\n" + "="*60)
    if success:
        log.success("Nutrilio source processing completed successfully!")
        log.info("📁 Outputs:")
        log.info(" • Health data: files/source_processed_files/nutrilio/nutrilio_health_processed.csv")
        log.info(" • Nutrition data: files/source_processed_files/nutrilio/nutrilio_nutrition_processed.csv")
        # Record successful run
        record_successful_run('source_nutrilio', 'active')
    else:
        log.error("Nutrilio source processing failed")
    log.info("="*60)
...
if __name__ == "__main__":
    ...
    log.info("Nutrilio Source Processing Tool")
    log.info("This tool downloads and processes Nutrilio data.")
    log.info("Note: Upload is handled by topic coordinators (Health/Nutrition)")
