...
def select_columns(df, name_val, data_type):
    ...
    path = f'files/source_processed_files/apple/apple_{name_val}.csv'

    log.normal(f"Processing {name_val}...")
    log.progress(f"Processing {name_val} using select_columns method...")
    ...
    log.normal_success(f"{name_val}: {len(df)} records, completed in {elapsed:.1f}s")
    return df
...
def expand_df(df, name_val, aggreg_method='sum'):
    ...
    log.info(f'{df.shape[0]} new rows to expand for {name_val}')
    for _, row in df.iterrows():
        new_df = pd.concat([new_df, row_expander_minutes(row, aggreg_method)], ignore_index=True)
    log.info(f'{df.shape[0]} rows expanded for {name_val} \n')
...
def expand_df_vectorized(df, name_val, aggreg_method='sum'):
    ...
    log.normal(f"Processing {name_val}...")
    log.milestone(f"Starting optimized expansion for {name_val}...")
    start_time = time.time()
    ...
    try:
        ...
        log.progress(f"{len(df_filtered)} new rows to expand for {name_val}")

        if len(df_filtered) == 0:
            log.success(f"No new data for {name_val}, using existing processed data")
            elapsed_time = time.time() - start_time
            log.normal_success(f"{name_val}: no new data, {len(old_df)} existing records ({elapsed_time:.1f}s)")
            return old_df.rename(columns={'val': name_val})

    except FileNotFoundError:
        log.progress(f"{len(df_filtered)} total rows to expand for {name_val} (new file)")
    ...
    log.progress(f"Expanding {len(df_filtered)} rows using vectorized operations...")
    ...
    for i in range(0, len(df_filtered), batch_size):
        ...
        if len(df_filtered) > batch_size:
            ...
            log.progress(f"Progress: {progress}/{len(df_filtered)} rows processed")
    ...
    # Performance summary
    elapsed_time = time.time() - start_time
    log.success(f"{name_val} expansion completed in {elapsed_time:.2f} seconds")
    log.success(f"Final dataset: {len(result_df)} aggregated daily records\n")
    log.normal_success(f"{name_val}: {len(result_df)} records, completed in {elapsed_time:.1f}s")
...
def download_apple_data():
    ...
    """
    log.info("🍎 Starting Apple Health data download...")

    log.info("Instructions:")
    log.info(" 1. Open the Health app on your iPhone")
    log.info(" 2. Tap your profile picture in the top right")
    log.info(" 3. Tap 'Export All Health Data'")
    log.info(" 4. Choose 'Export' and then select how to share the data")
    log.info(" 5. Save the export.zip file to your Downloads folder")
...
def move_apple_files():
    ...
    """
    log.info("📁 Moving Apple Health files...")

    # First, try to unzip the apple health file
    unzip_success = find_unzip_folder("apple")
    if not unzip_success:
        log.error("Failed to find or unzip Apple Health file")
        return False
    ...
    if move_success:
        log.success("Successfully moved Apple Health files to exports folder")
    else:
        log.error("Failed to move Apple Health files")
...
def create_apple_files():
    ...
    log.normal("Processing Apple Health metrics...")
    log.progress("Preparing timestamp data...")
    ...
    log.progress("Using optimized vectorized processing for metrics expansion...")
    ...
    log.progress(f"Merging all metrics datasets...")
    ...
    output_path = 'files/source_processed_files/apple/apple_processed.csv'
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    apple_df.to_csv(output_path, sep='|', index=False, encoding='utf-8')

    log.success(f"\n Apple Health processing completed!")
    log.info(f"⏱️  Total vectorized processing time: {total_elapsed:.2f} seconds")
    log.progress(f"Final merged dataset: {len(apple_df)} daily records")
    log.info(f"📅 Date range: {apple_df['date'].min()} to {apple_df['date'].max()}")
    log.normal_success(f"Apple Health: {len(apple_df)} daily records, processed in {total_elapsed:.1f}s")
...
def create_apple_file():
    ...
    Returns True if successful, False otherwise.
    """
    log.info("⚙️  Processing Apple Health data...")

    try:
        # Check if input files exist
        path_export = 'files/exports/apple_exports/apple_health_export/export.xml'
        if not os.path.exists(path_export):
            log.error(f"Apple Health export file not found: {path_export}")
            return False

        # Process the files using existing logic
        create_apple_files()

        log.success("Apple Health data processing complete!")
        return True

    except Exception as e:
        log.error(f"Error processing Apple Health data: {e}")
        return False
...
def full_apple_pipeline(auto_full=False, auto_process_only=False):
    """
    Complete Apple Health SOURCE PROCESSOR pipeline (NO UPLOAD).
    ...
    """
    log.info("\n" + "="*60)
    log.info("🍎 APPLE HEALTH SOURCE PROCESSOR")
    log.info("="*60)
    ...
    log.info("\n" + "="*60)
    if success:
        log.success("Apple Health source processor completed successfully!")
        # Record successful run with new tracking name
        record_successful_run('source_apple', 'active')
    else:
        log.error("Apple Health source processor failed")
    log.info("="*60)
...
if __name__ == "__main__":
    ...
    log.info("🍎 Apple Health Source Processing Tool")
    log.info("This tool downloads and processes Apple Health data.")
    log.info("Note: Upload is handled by the Health topic coordinator")
