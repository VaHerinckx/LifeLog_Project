    ...
    try:
        ...
        if not os.path.exists(DICT_PATH):
            with open(DICT_PATH, 'w') as f:
                json.dump({}, f)
            log.success(f"Created empty dictionary file: {DICT_PATH}")

        return True
    except Exception as e:
        log.error(f"Error checking required files: {e}")
        return False
...
def load_fit_files_dictionary():
    ...
    try:
        ...
        log.warning(f"Dictionary file not found or corrupted, creating new one")
        return {}
...
def save_fit_files_dictionary(dict_files):
    ...
    try:
        ...
        log.error(f"Error saving dictionary file: {e}")
        return False
...
def row_expander_minutes(row):
    ...
    try:
        ...
        log.error(f"Error expanding row: {e}")
        return pd.DataFrame()
...
def formatting_garmin_df(df, columns_datetime=None, columns_duration=None, columns_speed=None, columns_distance=None):
    ...
    try:
        ...
        log.error(f"Error formatting DataFrame: {e}")
        return df
...
def activity_summary_extract(path):
    ...
    try:
        log.progress("Processing activity summaries...")
        ...
        if overall_df.empty:
            log.warning("No activity data found in file")
            return pd.DataFrame()

        log.progress(f"Found {len(overall_df)} activities to process")
        ...
        if 'startTimeGmt' in df.columns:
            log.progress("Applying location-based timezone correction...")
        ...
        log.success(f"Activity summary processed: {len(df)} records saved")
        return df

    except Exception as e:
        log.error(f"Error processing activity summary: {e}")
        return pd.DataFrame()
...
def activity_splits_extract(df):
    ...
    try:
        log.progress("Processing activity splits...")

        if df.empty:
            log.warning("No activity data provided for splits processing")
            return pd.DataFrame()
            ...
            try:
                ...
                for _, row_split in activity_splits.iterrows():
                    ...
                    try:
                        ...
                        log.warning(f"Error processing measurement: {e}")
                        continue
                ...
                log.warning(f"Error processing activity splits: {e}")
                continue

        if activity_splits_full.empty:
            log.warning("No valid splits data found")
            return pd.DataFrame()

        log.progress(f"Processed splits from {processed_activities} running activities")
        ...
        log.progress("Applying timezone correction to splits data...")
        ...
        # Expand minutes
        log.info("⏱️  Expanding data by minutes...")
        new_df = pd.DataFrame()
        ...
        for _, row in df_filtered.iterrows():
            try:
                ...
                log.warning(f"Error expanding row: {e}")
                continue
        ...
        log.success(f"Activity splits processed: {len(new_df)} minute-level records saved")
        return new_df

    except Exception as e:
        log.error(f"Error processing activity splits: {e}")
        return pd.DataFrame()
...
def sleep_file(path):
    ...
    try:
        if not os.path.exists(path):
            log.warning(f"Sleep file not found: {path}")
            return pd.DataFrame()
        ...
        log.error(f"Error processing sleep file {path}: {e}")
        return pd.DataFrame()
...
def process_sleep_files():
    ...
    try:
        log.info("😴 Processing sleep data...")

        folder_path = "files/exports/garmin_exports/DI_CONNECT/DI-Connect-Wellness"

        if not os.path.exists(folder_path):
            log.error(f"Sleep data folder not found: {folder_path}")
            return False
        ...
        if not sleep_files:
            log.warning("No sleep data files found")
            return False

        log.info(f"📁 Found {len(sleep_files)} sleep data files")
        ...
        if df_sleep.empty:
            log.warning("No valid sleep data found")
            return False
        ...
        log.success(f"Sleep data processed: {processed_files} files, {len(df_sleep)} records saved")
        return True

    except Exception as e:
        log.error(f"Error processing sleep files: {e}")
        return False
...
def extract_fit_files_path():
    ...
    try:
        ...
        if not os.path.exists(base_path):
            log.warning(f"FIT files folder not found: {base_path}")
            return []
        ...
        log.info(f"📁 Found {len(fit_files)} FIT files")
        return fit_files

    except Exception as e:
        log.error(f"Error extracting FIT file paths: {e}")
        return []
...
def process_fit_file_data():
    ...
    try:
        log.progress("Processing FIT file data (stress, respiration, body battery, HRV)...")
        ...
        if os.path.exists(uploaded_files_folder):
            ...
            if zip_files:
                log.progress(f"Found {len(zip_files)} ZIP file(s) to extract...")
                for zip_filename in zip_files:
                    zip_file_path = os.path.join(uploaded_files_folder, zip_filename)
                    log.info(f"Extracting {zip_filename}...")
                    with zipfile.ZipFile(zip_file_path, 'r') as zip_ref:
                        zip_ref.extractall(fit_files_folder)
                    os.remove(zip_file_path)
                log.success(f"Extracted all ZIP files")

        fit_files = extract_fit_files_path()
        if not fit_files:
            log.warning("No FIT files found for processing")
            return False
        ...
        for fit_file in fit_files:
            ...
            try:
                ...
                if count_new_files % 100 == 0:
                    log.progress(f"Processed {count_new_files} new FIT files...")

            except Exception as e:
                log.warning(f"Error processing FIT file {fit_file}: {e}")
                continue
        ...
        log.error(f"Error processing FIT file data: {e}")
        import traceback
        traceback.print_exc()
        return False
    ...
    try:
        ...
        if data:
            ...
            if time_column in df.columns:
                ...
                if null_count > 0:
                    log.warning(f"Filtering out {null_count} records with null timestamps")
                    df = df.dropna(subset=[time_column])

                log.progress(f"Applying timezone correction to {data_name.lower()} data...")
            ...
            log.success(f"{data_name} data: {len(df)} records saved")
        elif not df_existing.empty:
            log.info(f"No new {data_name.lower()} data (existing: {len(df_existing)} records)")
        else:
            log.warning(f"No {data_name.lower()} data found")

    except Exception as e:
        log.error(f"Error saving {data_name.lower()} data: {e}")


def process_training_history():
    """Process training history data"""
    try:
        log.info("️ Processing training history...")

        folder_path = "files/exports/garmin_exports/DI_CONNECT/DI-Connect-Metrics"

        if not os.path.exists(folder_path):
            log.error(f"Training history folder not found: {folder_path}")
            return False
        ...
        if not files:
            log.warning("No training history files found")
            return False

        log.info(f"📁 Found {len(files)} training history files")
        ...
        if not dataframes:
            log.warning("No valid training history data found")
            return False
        ...
        if 'timestamp' in df.columns:
            log.progress("Applying timezone correction to training history...")
        ...
        log.success(f"Training history processed: {len(df)} records saved")
        return True

    except Exception as e:
        log.error(f"Error processing training history: {e}")
        return False
...
def download_garmin_data():
    ...
    """
    log.info("⌚ Starting Garmin data download...")

    urls = ['https://www.garmin.com/fr-BE/account/datamanagement/exportdata']
    open_web_urls(urls)

    log.info("Instructions:")
    log.info(" 1. Log into your Garmin Connect account")
    log.info(" 2. Go to Account Settings > Data Export")
    log.info(" 3. Request your data export")
    log.info(" 4. Wait for the export email (can take several hours)")
    log.info(" 5. Download and extract the ZIP file to Downloads folder")
    log.info(" 6. The file should be named like 'DI_CONNECT_FITNESS_METRICS_[timestamp].zip'")
...
def move_garmin_files():
    ...
    if move_success:
        log.success("Successfully moved Garmin files to exports folder")
        # Clear FIT files dictionary to force reprocessing of all files
        log.progress("Clearing FIT files dictionary for fresh processing...")
        save_fit_files_dictionary({})
    else:
        log.error("Failed to move Garmin files")
...
def create_garmin_files():
    ...
    Returns True if successful, False otherwise.
    """
    log.info("⚙️  Processing Garmin data...")
    ...
    try:
        ...
        if os.path.exists(activity_path):
            ...
            if not df.empty:
                log.success("Activity summaries processed successfully")
                success_count += 1

                # Process activity splits
                splits_result = activity_splits_extract(df)
                if not splits_result.empty:
                    log.success("Activity splits processed successfully")
                    success_count += 1
                else:
                    log.warning("Activity splits processing had issues")
            else:
                log.error("Failed to process activity summaries")
        else:
            log.error(f"Activity summary file not found: {activity_path}")
        ...
        log.normal(f"\n Processing Summary:")
        log.normal_success(f"Successful: {success_count}/{total_processes} processes")
        log.warning(f"{' All processes completed successfully!'if success_count == total_processes else 'Some processes had issues'}")

        return success_count > 0

    except Exception as e:
        log.error(f"Error in main processing: {e}")
        return False
...
def full_garmin_pipeline(auto_full=False, auto_process_only=False):
    ...
    """
    log.info("\n" + "="*60)
    log.info("⌚ GARMIN SOURCE PIPELINE")
    log.info("="*60)
    ...
    log.info("\n" + "="*60)
    if success:
        log.success("Garmin source pipeline completed!")
        log.info("Note: To upload to Drive, run the Fitness topic pipeline.")
        # Record successful run
        record_successful_run('source_garmin', 'active')
    else:
        log.error("Garmin pipeline failed")
    log.info("="*60)
    ...
    try:
        if data_type == 'activities':
            ...
            if os.path.exists(activity_path):
                ...
                log.error(f"File not found: {activity_path}")
                return False
            ...
            log.info("Available types: activities, splits, sleep, stress (includes respiration/body battery/HRV), training")
            return False

    except Exception as e:
        log.error(f"Error processing {data_type}: {e}")
        return False


if __name__ == "__main__":
    # Allow running this file directly
    log.info("⌚ Garmin Processing Tool")
    log.info("This tool helps you download, process, and upload Garmin data.")
