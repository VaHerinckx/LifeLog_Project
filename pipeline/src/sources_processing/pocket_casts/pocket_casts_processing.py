...
def identify_language_new_pods(df, list_new, dict_pod_language):
    ...
    log.info(f"Detecting language for {len(list_new)} new podcasts...")
    ...
    for pod in list_new:
        ...
        log.info(f"• {pod}: {detected_language}")

    with open(path_dict_language, 'w') as f:
        json.dump(dict_pod_language, f)

    log.success("Language detection completed")
...
def parse_table_history(table_str):
    ...
    if dropped_count > 0:
        log.warning(f"Dropped {dropped_count} rows with invalid 'modified at'timestamps")

    if len(df_history) == 0:
        log.error("Error: No valid episodes found in history data")
        return df_history
...
def missing_name_genre(df, list_new):
    ...
    for podcast_uuid in list_new:
        ...
        log.info(f"\n{'='*60}")
        log.info(f"New podcast UUID: {podcast_uuid}")
        log.info(f"{'='*60}")
        ...
        for i, ep in enumerate(sample_episodes, 1):
            log.info(f"{i}. {ep[:70]}..." if len(ep) > 70 else f" {i}. {ep}")

        # Try auto-discovery first (iTunes API)
        log.info(f"\n{'='*60}")
        log.info("ATTEMPTING AUTO-DISCOVERY (using iTunes API)...")
        log.info(f"{'='*60}")
        ...
        if discovered_info:
            ...
            log.success(f"\n Auto-discovery successful!")
            log.info(f"Name: {new_podcast_data['podcast_name']}")
            log.info(f"Genre: {new_podcast_data['genre']}")
        else:
            # Fall back to manual input
            log.info(f"\n{'='*60}")
            log.info("AUTO-DISCOVERY FAILED - MANUAL INPUT REQUIRED")
            log.info(f"{'='*60}")
            ...
            if backup_genre is None:
                ...
                log.info(f"0. [Enter new genre manually]")
                ...
                while True:
                    try:
                        ...
                        if choice_num == 0:
                            ...
                            log.error(f"Invalid choice. Please enter a number between 0 and {len(existing_genres)}")
                    except ValueError:
                        log.error(f"Invalid input. Please enter a number.")
        ...
        log.success(f"Saved to mapping file: {new_podcast_data['podcast_name']}\n")
...
def add_columns(df):
    ...
    total_podcasts = df["podcast"].nunique()
    already_mapped = total_podcasts - len(list_new)
    if len(list_new) > 0:
        log.normal(f"Podcast mapping: {already_mapped} already mapped, {len(list_new)} new podcast(s) need discovery")
        missing_name_genre(df, list_new)
        df = retrieve_name_genre(df)
    else:
        log.normal(f"Podcast mapping: all {already_mapped} podcasts already mapped")
    return df, list_new
...
def get_podcast_info(podcast_name):
    ...
    try:
        ...
        log.error(f"Error fetching info for {podcast_name}: {str(e)}")
        return None
...
def auto_discover_podcast_info(podcast_uuid, df):
    ...
    log.debug(f"Found {len(sample_titles)} episode titles to search:")
    for i, title in enumerate(sample_titles[:3], 1):
        log.info(f"{i}. {title[:60]}..." if len(title) > 60 else f" {i}. {title}")
    ...
    for title in sample_titles:
        ...
        if info and info.get('itunes_name'):
            ...
            log.info(f"Podcast: {info['itunes_name']}")
            log.info(f"Creator: {info['artist']}")
            log.info(f"Genre: {info['genre']}")

            # Ask user to confirm
            confirm = input(f"\n   Is this the correct podcast? (y/n): ").strip().lower()

            if confirm == 'y':
                log.success(f"Confirmed: {info['itunes_name']}")
                # Use iTunes name as the podcast name
                info['podcast_name'] = info['itunes_name']
                return info
            else:
                log.info(" Trying next search...")
                continue

    log.error("Auto-discovery failed - no match found")
    return None
...
def load_artwork_cache():
    ...
    try:
        ...
        log.info("Artwork cache file not found, creating new one")
        return {}
    except json.JSONDecodeError:
        log.warning("Artwork cache file corrupted, starting fresh")
        return {}
...
def manual_genre_override():
    ...
    """
    log.info("\n" + "="*60)
    log.info("MANUAL GENRE OVERRIDE")
    log.info("="*60)
    ...
    try:
        ...
        log.error("Processed file not found. Please process data first.")
        return False
    ...
    if not matches:
        log.error(f"No podcasts found matching '{search_term}'")
        return False
    ...
    if len(matches) > 1:
        ...
        for i, podcast in enumerate(matches, 1):
            ...
            log.info(f"{i}. {podcast}")
            log.info(f"   Current genre: {current_genre}")
    ...
    log.info(f"Podcast: {podcast_name}")
    log.info(f"Old genre: {current_genre}")
    log.info(f"New genre: {new_genre}")
...
def manual_artwork_override():
    ...
    """
    log.info("\n" + "="*60)
    log.info("🎨 MANUAL ARTWORK OVERRIDE")
    log.info("="*60)
    ...
    if len(matches) > 1:
        ...
        for i, podcast in enumerate(matches, 1):
            ...
            log.info(f"{i}. {podcast}")
            log.info(f"   Current: {url_display}")
    ...
    log.progress(f"\n Podcast: {podcast_name}")
    log.info(f"Current artwork URL: {current_url}")
    ...
    log.info(f"Podcast: {podcast_name}")
    log.info(f"New URL: {new_url[:60]}..." if len(new_url) > 60 else f" New URL: {new_url}")
...
def manual_itunes_search():
    ...
    """
    log.info("\n" + "="*60)
    log.progress("MANUAL ITUNES API SEARCH")
    log.info("="*60)
    ...
    try:
        ...
        log.error("iTunes API cache not found. Please run the pipeline first.")
        return False
    ...
    if len(matches) > 1:
        ...
        for i, podcast in enumerate(matches, 1):
            ...
            log.info(f"{i}. {podcast}")
            log.info(f"   Current iTunes match: {current_itunes}")
    ...
    try:
        ...
        if data['resultCount'] == 0:
            log.error("No results found from iTunes API")
            return False

        # Display all results
        log.progress(f"\n Found {data['resultCount']} result(s) from iTunes API:")
        log.info(f"0. [Cancel - don't update]")
        ...
        for i, podcast in enumerate(data['results'][:10], 1):  # Limit to 10 results
            ...
            log.info(f"{i}. {itunes_name}")
            log.info(f"   Creator: {artist}")
        ...
        while True:
            try:
                ...
                if choice_num == 0:
                    log.error("Cancelled - no updates made")
                    return False
                elif 1 <= choice_num <= len(results):
                    selected_info = results[choice_num - 1]
                    break
                else:
                    log.error(f"Invalid choice. Please enter a number between 0 and {len(results)}")
            except ValueError:
                log.error(f"Invalid input. Please enter a number.")

    except Exception as e:
        log.error(f"Error fetching from iTunes API: {str(e)}")
        return False
    ...
    log.info(f"Podcast: {podcast_name}")
    log.info(f"New iTunes match: {selected_info['itunes_name']}")
    log.info(f"New genre: {selected_info['genre']}")
...
def enrich_podcast_data(df):
    ...
    try:
        ...
        log.warning("iTunes API cache file not found, creating new one")
        ...
        log.warning("iTunes API cache has formatting issues, attempting recovery...")
        try:
            api_results_df = pd.read_csv(ITUNES_CACHE_FILE, sep="|", quoting=1, escapechar='\\')
        except:
            # If still fails, start fresh
            log.error("Could not parse existing iTunes API cache, starting fresh")
            api_results_df = pd.DataFrame(columns=['podcast_name', 'itunes_name', 'artist', 'artwork_large', 'genre', 'feed_url'])

    # 3. Get API data for new podcasts
    counter = 0
    new_podcasts_to_fetch = []
    for podcast in unique_podcasts:
        if pd.isna(podcast):
            continue
        if api_results_df.empty or podcast not in api_results_df['podcast_name'].values:
            new_podcasts_to_fetch.append(podcast)

    already_cached = len([p for p in unique_podcasts if not pd.isna(p)]) - len(new_podcasts_to_fetch)
    if new_podcasts_to_fetch:
        log.normal(f"iTunes API discovery: {already_cached} podcasts already cached, {len(new_podcasts_to_fetch)} new to fetch")
    else:
        log.normal(f"iTunes API discovery: all {already_cached} podcasts already cached, 0 new to fetch")
    ...
    for podcast in unique_podcasts:
        ...
        if api_results_df.empty or podcast not in api_results_df['podcast_name'].values:
            log.progress(f"Fetching Itunes API data for {podcast}")
            ...
            if info:
                ...
                log.info(f"Results for {podcast} were fetched")
                matched_count += 1
            else:
                not_found_count += 1
        if counter%10 == 0:
            api_results_df.to_csv(ITUNES_CACHE_FILE, index=False, sep="|", quoting=1, escapechar='\\')
            log.info("Temporary API results saved")

    if new_podcasts_to_fetch:
        log.normal_success(f"iTunes API results: {matched_count} matched, {not_found_count} not found")

    # Save updated API results with proper quoting to handle special characters
    api_results_df.to_csv(ITUNES_CACHE_FILE, index=False, sep="|", quoting=1, escapechar='\\')
    log.info("All API results saved")
...
def create_pocket_casts_file():
    ...
    try:
        ...
        if not os.path.exists(EXPORT_DATA_FILE):
            log.error(f"Export file not found: {EXPORT_DATA_FILE}")
            return False
        ...
        # End summary at NORMAL verbosity
        num_episodes = len(df)
        num_podcasts = df['podcast_name'].nunique()
        date_range_start = df['listened_date'].min()
        date_range_end = df['listened_date'].max()
        log.normal_success(f"Pocket Casts processed: {num_episodes:,} episodes across {num_podcasts} podcasts ({date_range_start} to {date_range_end})")

        # Generate website files
        return True

    except Exception as e:
        log.error(f"Error processing Pocket Casts data: {str(e)}")
        import traceback
        traceback.print_exc()
        return False
...
def download_pocket_casts_data():
    ...
    log.info("1. Email Pocket Casts support or use their GDPR request form")
    log.info("2. Wait for them to prepare your data export (usually 1-3 days)")
    log.info("3. Download the zip file they email you")
    log.info("4. Save the zip file to your Downloads folder")
    ...
    if choice == 'y':
        log.success("Ready to proceed with processing")
        return True
    else:
        log.warning("Please download the export file first, then run this pipeline again")
        return False
...
def move_pocket_casts_files():
    ...
    try:
        ...
        log.error(f"Error moving files: {str(e)}")
        return False
...
def full_pocket_casts_pipeline(auto_full=False, auto_process_only=False):
    ...
    """
    log.info("\n" + "="*60)
    log.info("🎙️  POCKET CASTS SOURCE DATA PIPELINE")
    log.info("="*60)
    ...
    if auto_process_only:
        ...
        log.prompt("3. Manual genre override")
        log.prompt("4. Manual artwork override")
    ...
    try:
        if choice == "1":
            ...
            log.warning("Run option 2 to reprocess with new genres")
            return True
            ...
            log.warning("Run option 2 to reprocess with new artwork")
            return True
    ...
    log.info("\n" + "="*60)
    if success:
        log.success("Pocket Casts source pipeline completed successfully!")
        log.info("Note: To upload to Drive, run the Podcasts topic pipeline.")
        record_successful_run('source_pocket_casts', 'active')
    else:
        log.error("Pocket Casts source pipeline failed")
    log.info("="*60)
...
def process_pocket_casts_export(upload="Y"):
    ...
    log.warning('This is a legacy function. Please use full_pocket_casts_pipeline() instead.')
    log.info('Starting the processing of the Pocket Casts export \n')
    find_unzip_folder("pocket_casts")
    clean_rename_move_folder("files/exports", os.path.expanduser("~/Downloads"), "pocket_casts_export_unzipped", "pocket_casts_exports")
    create_pocket_casts_file()
    log.info('Pocket Cast processed files were created \n')
