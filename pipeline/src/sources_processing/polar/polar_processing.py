...
def process_polar_summary(df):
    ...
    try:
        ...
        log.error(f"Error processing Polar summary: {e}")
        return pd.DataFrame()
...
def process_polar_splits(df):
    """Reformats the columns to have similar structure as Garmin output"""
    try:
        log.progress("Processing splits data (this may take a moment)...")
        ...
        log.error(f"Error processing Polar splits: {e}")
        return pd.DataFrame()
...
def create_polar_files():
    ...
    """
    log.info("⚙️  Processing Polar data...")

    folder_path = 'files/exports/polar_exports'

    if not os.path.exists(folder_path):
        log.error(f"Polar exports folder not found: {folder_path}")
        return False
    ...
    try:
        ...
        if not zip_files:
            log.warning("No Polar ZIP files found in exports folder")
            return False

        log.info(f"📁 Found {len(zip_files)} Polar export files")
        ...
        if not summary_dfs:
            log.warning("No valid Polar data found")
            return False
        ...
        log.progress(f"Processing {len(summary_df)} activities and {len(detail_df)} detail records")
        ...
        if not processed_summary.empty:
            ...
            log.success(f"Polar summary processed: {len(processed_summary)} records saved")
        ...
        if not processed_splits.empty:
            ...
            log.success(f"Polar splits processed: {len(processed_splits)} records saved")

        return True

    except Exception as e:
        log.error(f"Error processing Polar data: {e}")
        return False
...
def full_polar_pipeline(auto_process_only=False):
    ...
    """
    log.info("\n" + "="*60)
    log.info("⌚ POLAR SOURCE PIPELINE")
    log.info("="*60)
    ...
    log.info("\n" + "="*60)
    if success:
        log.success("Polar source pipeline completed!")
        log.info("Note: Polar data will be merged when running the Fitness topic pipeline.")
        record_successful_run('source_polar', 'legacy')
    else:
        log.error("Polar pipeline failed")
    log.info("="*60)
...
if __name__ == "__main__":
    log.info("⌚ Polar Processing Tool")
    log.info("This tool processes existing Polar export data.")
    full_polar_pipeline()
